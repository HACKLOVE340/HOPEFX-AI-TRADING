...
const LEVEL_COLORS: Record<string,string> = { bronze:'#cd7f32', silver:'var(--text-dim)', gold:'#f59e0b', platinum:'var(--ai-model)' };
    ...
    <div className="page-content">
      ...
      {activeTab==='commissions'&&(
        <div style={st.card}>
          ...
          {commissions.length===0&&!subErrors.commissions?(<p style={{color:'var(--text-muted)',fontSize:14}}>No commissions yet.</p>):(
            ...
            <tbody>{commissions.map(c=>(<tr key={c.commission_id} style={st.tr}><td style={st.td}>{c.period}</td><td style={{...st.td,color:'var(--gain)',fontWeight:600}}>{fmtUSD(c.amount)}</td><td style={st.td}>{statusBadge(c.status)}</td><td style={st.td}>{c.paid_at?new Date(c.paid_at).toLocaleDateString():'—'}</td></tr>))}</tbody></table>
          )}
        </div>
      )}
...
const st:Record<string,React.CSSProperties>={
  ...
  refreshBtn:{background:'transparent',border:'1px solid #334155',borderRadius:6,color:'var(--text-dim)',cursor:'pointer',fontSize:13,padding:'6px 12px'},
