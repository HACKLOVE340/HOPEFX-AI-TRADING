...
def test_execution_invariants():
    ...
    ok = execution.verify_pre_trade_gate(
        prediction_exists=True, confidence_ok=True, risk_approved=True,
        capital_available=True, market_open=True, broker_reachable=True, kill_switch_active=False)
    assert ok == []
    blocked = execution.verify_pre_trade_gate(
        prediction_exists=True, confidence_ok=True, risk_approved=True,
        capital_available=True, market_open=True, broker_reachable=True, kill_switch_active=True)
    assert blocked  # kill switch blocks
    failed = execution.verify_pre_trade_gate(
        prediction_exists=True, confidence_ok=True, risk_approved=False,
        capital_available=True, market_open=True, broker_reachable=True, kill_switch_active=False)
