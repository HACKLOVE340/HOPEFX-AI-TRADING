...
class TestChartReplayExtended:
    ...
    def test_callback_error_does_not_propagate(self, engine, session):
        engine.register_callback(
            "on_trade", lambda *a: 1 / 0
        )  # intentional ZeroDivisionError — tests callback isolation
        # Should not crash
...
class TestNoCodeBuilderExtended:
    ...
    def test_parse_plain_english_buy(self, builder):
        result = builder.parse_plain_english("buy when rsi is below 30", "XAUUSD", "1H")
...
class TestAIExplainabilityExtended:
    ...
    def mock_model(self):
        ...
        model.predict_proba = MagicMock(return_value=np.array([[0.1, 0.7, 0.2]]))
