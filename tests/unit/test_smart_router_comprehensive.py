...
class TestBrokerState:
    ...
    def test_routing_score_circuit_open_excluded_from_ranking(self):
        r = _router()
        r.add_broker("open", _broker_inst())
        r.add_broker("closed", _broker_inst())
    ...
    def test_routing_score_lower_latency_higher_score(self):
        fast = BrokerState(broker_id="f")
        fast.ema_latency_ms = 5.0
        slow = BrokerState(broker_id="s")
        slow.ema_latency_ms = 500.0
    ...
    def test_routing_score_lower_slippage_higher_score(self):
        good = BrokerState(broker_id="g")
        good.avg_slippage_bps = 0.5
        bad = BrokerState(broker_id="b")
        bad.avg_slippage_bps = 50.0
...
class TestSmartRouterRankBrokers:
    ...
    def test_rank_sorted_by_score_descending(self):
        r = _router()
        r.add_broker("fast", _broker_inst())
        r.add_broker("slow", _broker_inst())
        r._states["fast"].ema_latency_ms = 5.0
        r._states["slow"].ema_latency_ms = 500.0
...
class TestSmartRouterPreRouteGate:
    def test_normal_order_returns_none(self):
        r = _router()
        result = r._pre_route_gate(spread_bps=2.0, sentiment_score=0.0, impact_score=0.0, direction="long", ofi=0.0)
    ...
    def test_spread_too_wide_returns_reason(self):
        ...
        result = r._pre_route_gate(
            spread_bps=_MAX_SPREAD_BPS + 1, sentiment_score=0.0, impact_score=0.0, direction="long", ofi=0.0
        )
    ...
    def test_extreme_sentiment_returns_reason(self):
        ...
        r = _router()
        result = r._pre_route_gate(
            spread_bps=1.0, sentiment_score=_SENT_BLACKOUT_THRESH + 0.1, impact_score=0.0, direction="long", ofi=0.0
        )
    ...
    def test_high_impact_returns_reason(self):
        ...
        r = _router()
        result = r._pre_route_gate(
            spread_bps=1.0, sentiment_score=0.0, impact_score=_IMPACT_BLACKOUT + 0.1, direction="long", ofi=0.0
        )
        assert result is not None and "impact" in result

    def test_adverse_ofi_long_returns_reason(self):
        r = _router()
        result = r._pre_route_gate(spread_bps=1.0, sentiment_score=0.0, impact_score=0.0, direction="long", ofi=-0.9)
        assert result is not None and "ofi" in result

    def test_adverse_ofi_short_returns_reason(self):
        r = _router()
        result = r._pre_route_gate(spread_bps=1.0, sentiment_score=0.0, impact_score=0.0, direction="short", ofi=0.9)
        assert result is not None and "ofi" in result

    def test_ofi_aligned_long_returns_none(self):
        r = _router()
        result = r._pre_route_gate(spread_bps=1.0, sentiment_score=0.0, impact_score=0.0, direction="long", ofi=0.5)
        assert result is None
...
class TestSmartRouterRouteAndExecute:
    ...
    async def test_route_success(self):
        r = _router()
        r.add_broker("b1", _broker_inst(status="filled"))
    ...
    async def test_route_broker_exception_records_error(self):
        r = _router()
        b = MagicMock()
        b.place_order = AsyncMock(side_effect=ConnectionError("timeout"))
        r.add_broker("b1", b)
        await r.route_and_execute(_order())
    ...
    async def test_route_fallback_to_second_broker(self):
        r = _router()
        bad = MagicMock()
        bad.place_order = AsyncMock(side_effect=ConnectionError("timeout"))
    ...
    async def test_route_all_brokers_fail_returns_rejected(self):
        r = _router()
        b = MagicMock()
        b.place_order = AsyncMock(return_value={"status": "rejected", "reason": "margin"})
        r.add_broker("b1", b)
        result = await r.route_and_execute(_order())
        assert result["status"] == "rejected"
    ...
    async def test_route_pre_gate_rejection_returns_rejected(self):
        ...
        order = _order()
        order["sentiment"] = _SENT_BLACKOUT_THRESH + 0.5
