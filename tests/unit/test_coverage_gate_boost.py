...
class TestPerformanceMetrics:
    ...
    def test_sortino_no_downside(self):
        ...
        df = pd.DataFrame(
            {"equity": [100_000, 101_000, 102_000, 103_000]}, index=pd.date_range("2024-01-01", periods=4)
        )
        m = PerformanceMetrics(df, pd.DataFrame(), 100_000.0)
        assert m.calculate_sortino_ratio() == 0.0
    ...
    def test_calmar_zero_drawdown(self):
        ...
        df = pd.DataFrame({"equity": [100_000, 101_000, 102_000]}, index=pd.date_range("2024-01-01", periods=3))
        m = PerformanceMetrics(df, pd.DataFrame(), 100_000.0)
...
class TestWalkForwardEngine:
    ...
    def test_detect_overfit_true(self):
        ...
        e = WalkForwardEngine()
        assert (
            e._detect_overfit({"sharpe_ratio": 2.0, "total_return": 0.3}, {"sharpe_ratio": 0.5, "total_return": 0.05})
            is True
        )
    ...
    def test_detect_overfit_false(self):
        ...
        e = WalkForwardEngine()
        assert (
            e._detect_overfit({"sharpe_ratio": 1.0, "total_return": 0.1}, {"sharpe_ratio": 0.9, "total_return": 0.09})
            is False
        )
