        ...
        class MockBroker(BasePropFirmBroker):
            async def get_metrics(self):
                return PropFirmMetrics(
                    ...
                    remaining_daily_loss=0.0,  # exhausted
                    monthly_loss_limit=10_000,
                    remaining_monthly_loss=5_000,
                )
        ...
        class MockBroker(BasePropFirmBroker):
            async def get_metrics(self):
                return PropFirmMetrics(
                    ...
                    equity=101_000,
                    used_margin=0,
                    available_margin=101_000,
                    profit_loss=1_000,
                    profit_loss_percentage=1.0,
                    daily_drawdown=100,
                    daily_drawdown_percentage=0.1,
                    monthly_drawdown=100,
                    monthly_drawdown_percentage=0.1,
                    remaining_days=25,
                    trading_phase=TradingPhase.CHALLENGE,
                    trades_completed=5,
                    win_rate=0.6,
                    ...
                    account_status=AccountStatus.ACTIVE,
                    daily_loss_limit=5_000,
                    remaining_daily_loss=4_900,
                    monthly_loss_limit=10_000,
                    remaining_monthly_loss=9_900,
                )
...
class TestFTMOBrokerSignature:
    ...
    def test_generate_signature_with_data(self):
        ...
        broker = FTMOBroker("key", "secret", "acc", sandbox=True)
        headers = broker._generate_signature("POST", "/orders", data={"symbol": "EUR/USD", "side": "BUY"})
        assert "Authorization" in headers
...
class TestPropFirmFactory:
    def test_create_ftmo_broker(self):
        ...
        broker = PropFirmFactory.create_broker(PropFirmType.FTMO, "key", "secret", "acc", sandbox=True)
    ...
    def test_create_the5ers_broker(self):
        ...
        broker = PropFirmFactory.create_broker(PropFirmType.THE5ERS, "key", "secret", "acc", sandbox=True)
    ...
    def test_create_myforexfunds_broker(self):
        ...
        broker = PropFirmFactory.create_broker(PropFirmType.MYFOREXFUNDS, "key", "secret", "acc", sandbox=True)
    ...
    def test_create_topstep_broker(self):
        ...
        broker = PropFirmFactory.create_broker(PropFirmType.TOPSTEP, "key", "secret", "acc", sandbox=True)
