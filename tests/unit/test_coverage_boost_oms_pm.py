...
class TestOMSKillSwitch:
    ...
    def test_submit_blocked_when_kill_switch_active(self):
        from execution.oms import OrderLifecycleManager, Order, OrderStatus
        ...
        order = mgr.create_order(symbol="XAUUSD", side="BUY", order_type="MARKET",
                                  quantity=Decimal("1"))
...
class TestOMSAsyncSubmit:
    ...
    async def test_async_submit_broker_accepted(self):
        ...
        class _FakeBroker:
            async def place_order(self, req): return {"status": "accepted"}
    ...
    async def test_async_submit_broker_rejected(self):
        ...
        class _FakeBroker:
            async def place_order(self, req): return {"status": "rejected", "reason": "TEST"}
    ...
    async def test_async_submit_broker_raises(self):
        ...
        class _FakeBroker:
            async def place_order(self, req): raise RuntimeError("network error")
...
class TestOMSOrderBook:
    ...
    def test_get_order_book_with_orders(self):
        ...
        mgr = OrderLifecycleManager()
        order = mgr.create_order(symbol="XAUUSD", side="BUY", order_type="LIMIT",
...
class TestOMSEventBus:
    ...
    async def test_transition_with_event_bus(self):
        ...
        class _FakeEvent:
            @staticmethod
            def create(name, src, data): return (name, src, data)

        class _FakeBus:
            async def publish(self, evt): published.append(evt)
...
class TestOMSBracketIceberg:
    ...
    def test_create_bracket_order(self):
        ...
        entry = Order(symbol="XAUUSD", side="BUY", order_type="LIMIT",
                      quantity=Decimal("1"), price=Decimal("2000"))
    ...
    def test_create_oco(self):
        from execution.oms import Order
        mgr, oms = self._complex_mgr()
        o1 = Order(symbol="XAUUSD", side="BUY", order_type="LIMIT",
                   quantity=Decimal("1"), price=Decimal("2000"))
        o2 = Order(symbol="XAUUSD", side="SELL", order_type="STOP",
                   quantity=Decimal("1"), stop_price=Decimal("1980"))
...
class TestPositionManagerRedis:
    ...
    async def test_open_redis_rollback_on_error(self):
        ...
        class _FakeStore:
            async def save_position(self, d): raise ConnectionError("redis down")
            async def remove_position(self, s): pass
            async def load_state_on_boot(self): return {"positions": []}
    ...
    async def test_close_redis_rollback_on_error(self):
        ...
        class _FakeStore:
            async def save_position(self, d): pass
            async def remove_position(self, s): raise ConnectionError("redis down")
            async def load_state_on_boot(self): return {"positions": []}
    ...
    async def test_restore_from_redis_error_returns_zero(self):
        ...
        class _FakeStore:
            async def load_state_on_boot(self): raise ConnectionError("down")
