...
class TestPosition:
    def _make(self, side="long", qty=10.0, entry=1950.0, current=1950.0):
        ...
        return Position(id="p1", symbol="XAUUSD", side=side, quantity=qty, entry_price=entry, current_price=current)
...
class TestPositionTracker:
    def _make_pos(self, pid="p1", symbol="XAUUSD", side="long"):
        ...
        return Position(id=pid, symbol=symbol, side=side, quantity=1.0, entry_price=1950.0, current_price=1950.0)
...
class TestPartialFillState:
    def test_add_fill_updates_avg_price(self):
        ...
        state = PartialFillState(parent_id="p1", symbol="XAUUSD", side="long", target_lots=1.0)
    ...
    def test_fill_pct(self):
        ...
        state = PartialFillState(parent_id="p1", symbol="XAUUSD", side="long", target_lots=2.0)
        state.add_fill(1.0, 1950.0)
...
class TestBrokerCircuitBreaker:
    def _make(self, max_failures=3, reset_timeout=60.0, half_open_max=2):
        ...
        return BrokerCircuitBreaker(
            "test_broker", max_failures=max_failures, reset_timeout=reset_timeout, half_open_max=half_open_max
        )
