...
class TestAsyncExecutionEngine:
    ...
    def test_fill_dataclass(self):
        ...
        f = Fill(order_id="o1", symbol="XAUUSD", quantity=1.0, price=2350.0,
                 timestamp=datetime.now(UTC), side="buy")
        assert f.fees == 0.0
...
class TestFIXDataclasses:
    ...
    def test_fix_order_with_limit(self):
        from execution.fix_adapter import FIXOrder, FIXSide, FIXOrdType
        o = FIXOrder(symbol="GC", side=FIXSide.SELL, quantity=2.0,
                     ord_type=FIXOrdType.LIMIT, price=2400.0)
    ...
    def test_fix_fill_report_creation(self):
        ...
        f = FIXFillReport(cl_ord_id="t1", order_id="o1",
                          exec_type=FIXExecType.FILL,
                          symbol="GC", side=FIXSide.BUY,
                          filled_qty=1.0, avg_px=2350.0,
                          leaves_qty=0.0, cum_qty=1.0)
...
class TestFIXAdapterInit:
    def _adapter(self):
        ...
        return FIXAdapter(host="127.0.0.1", port=9876,
                          sender_comp_id="TEST", target_comp_id="CME",
                          username="", password="")
...
class TestTradeExecutorAdditional:
    def _executor(self):
        ...
        broker.place_order = AsyncMock(return_value=MagicMock(
            id="f1", filled_quantity=1.0, average_price=2350.0,
            status=MagicMock(value="filled"), commission=2.0))
        ...
        with patch("infrastructure.metrics.get_metrics_registry", return_value=MagicMock(
