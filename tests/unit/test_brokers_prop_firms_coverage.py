...
def _ftmo_metrics_payload():
    return {
        "accountBalance": 100_000.0, "equity": 101_000.0,
        "usedMargin": 2_000.0, "availableMargin": 99_000.0,
        "profitLoss": 1_000.0, "profitLossPercentage": 1.0,
        "dailyDrawdown": 100.0, "dailyDrawdownPercentage": 0.1,
        "monthlyDrawdown": 200.0, "monthlyDrawdownPercentage": 0.2,
        "remainingDays": 25, "phase": "challenge",
        "tradesCompleted": 5, "winRate": 0.6,
        "largestWin": 300.0, "largestLoss": 100.0,
        "consecutiveLosses": 1, "maxConsecutiveLosses": 3,
        "status": "active", "dailyLossLimit": 5_000.0,
        "remainingDailyLoss": 4_900.0, "monthlyLossLimit": 10_000.0,
...
class TestPropFirmInitModule:
    ...
    def test_prop_firm_config_fields(self):
        ...
        cfg = PropFirmConfig(
            firm_id="ftmo", api_key="k", secret_key="s",
            account_id="acc", tier=PropFirmTier.PROFESSIONAL,
...
class TestFTMOBrokerPlaceOrder:
    ...
    async def test_place_order_risk_violation_raises(self):
        from brokers.prop_firms.all_brokers import (
            AccountStatus, FTMOBroker, PropFirmMetrics,
...
class TestFTMOBrokerTrades:
    ...
    async def test_get_open_trades_success(self):
        ...
        broker = FTMOBroker("key", "secret", "acc", sandbox=True)
        payload = {"trades": [
            {"tradeId": "t1", "symbol": "EUR/USD", "side": "BUY",
             "entryPrice": "1.1000", "quantity": "1.0",
    ...
    async def test_get_trade_history_success(self):
        ...
        broker = FTMOBroker("key", "secret", "acc", sandbox=True)
        payload = {"trades": [
            {"tradeId": "t2", "symbol": "XAU/USD", "side": "SELL",
             "entryPrice": "1950.0", "exitPrice": "1940.0",
             "quantity": "1.0", "pnl": "100.0", "pnlPercentage": "0.1",
...
def _the5ers_metrics_payload():
    return {
        "balance": 50_000.0, "equity": 51_000.0,
        "usedMargin": 1_000.0, "availableMargin": 50_000.0,
        "profitLoss": 1_000.0, "profitLossPercent": 2.0,
        "dailyDD": 50.0, "dailyDDPercent": 0.1,
        "monthlyDD": 100.0, "monthlyDDPercent": 0.2,
        "daysRemaining": 20, "phase": "challenge",
        "totalTrades": 3, "winRate": 60.0,
        "bestTrade": 200.0, "worstTrade": 50.0,
        "consecutiveLosses": 0, "status": "active",
        "dailyLimit": 2_500.0, "remainingDaily": 2_450.0,
        "monthlyLimit": 5_000.0, "remainingMonthly": 4_900.0,
    }
...
def _mff_metrics_payload():
    ...
    return {
        "balance": 25_000.0, "equity": 25_500.0,
        "marginUsed": 500.0, "marginAvailable": 25_000.0,
        "profit": 500.0, "profitPercent": 2.0,
        "dailyLoss": 25.0, "dailyLossPercent": 0.1,
        "monthlyLoss": 50.0, "monthlyLossPercent": 0.2,
        "daysLeft": 15, "level": "challenge",
        "closedTrades": 2, "winPercent": 50.0,
        "maxProfit": 100.0, "maxLoss": 25.0,
        "losingStreak": 0, "status": "active",
        "dailyLossLimit": 1_250.0, "remainingDailyLoss": 1_225.0,
        "monthlyLossLimit": 2_500.0, "remainingMonthlyLoss": 2_450.0,
    }
...
def _topstep_metrics_payload():
    ...
    return {
        "cash": 150_000.0, "totalValue": 151_000.0,
        "marginUsed": 2_000.0, "buyingPower": 149_000.0,
        "netProfit": 1_000.0, "returnPercent": 0.67,
        "dailyDrawdown": 100.0, "dailyDrawdownPercent": 0.07,
        "monthDrawdown": 200.0, "monthDrawdownPercent": 0.13,
        "daysRemaining": 30, "phase": "funded",
        "totalTrades": 10, "winRate": 70.0,
        "largestWin": 500.0, "largestLoss": 200.0,
        "consecutiveLosses": 0, "status": "active",
        "dailyLossLimit": 3_000.0, "remainingDailyLimit": 2_900.0,
        "monthLossLimit": 4_500.0, "remainingMonthLimit": 4_300.0,
    }
...
class TestPropFirmGuard:
    def test_no_op_when_config_missing(self, tmp_path):
        ...
        import brokers.prop_firms.guard as guard
        guard._config = None
    ...
    def test_raises_on_daily_dd_breach(self, tmp_path):
        ...
        try:
            ...
            pass  # Expected to raise HTTPException or similar
...
class TestThe5ersBrokerSuccess:
    ...
    async def test_get_open_trades_success(self):
        ...
        broker = The5ersBroker("key", "secret", "acc", sandbox=True)
        payload = {"positions": [
            {"positionId": "p1", "instrument": "EUR/USD", "direction": "BUY",
             "openPrice": "1.1000", "volume": "1.0",
    ...
    async def test_get_trade_history_success(self):
        ...
        broker = The5ersBroker("key", "secret", "acc", sandbox=True)
        payload = {"trades": [
            {"tradeId": "t1", "instrument": "EUR/USD", "direction": "BUY",
             "openPrice": "1.1000", "closePrice": "1.1050",
             "volume": "1.0", "pnl": "50.0",
...
class TestMyForexFundsBrokerSuccess:
    ...
    async def test_get_open_trades_success(self):
        ...
        payload = {"openTrades": [
            {"id": "pos1", "pair": "XAU/USD", "action": "BUY",
             "openPrice": "1950.0", "lots": "0.5",
             "openTime": "2025-01-01T10:00:00Z"}
        ]}
        broker.session = _mock_session(200, payload)
        trades = await broker.get_open_trades()
    ...
    async def test_get_trade_history_success(self):
        ...
        payload = {"closedTrades": [
            {"id": "t1", "pair": "XAU/USD", "action": "BUY",
             "openPrice": "1950.0", "closePrice": "1960.0",
             "lots": "0.5", "profit": "50.0",
             "openTime": "2025-01-01T09:00:00Z",
             "closeTime": "2025-01-01T10:00:00Z"}
        ]}
        broker.session = _mock_session(200, payload)
...
class TestTopStepBrokerSuccess:
    ...
    async def test_get_open_trades_success(self):
        ...
        payload = {"positions": [
            {"id": "pos1", "symbol": "ES", "action": "Long",
             "averagePrice": "4500.0", "quantity": "1",
             "openTime": "2025-01-01T10:00:00Z"}
        ]}
        broker.session = _mock_session(200, payload)
        trades = await broker.get_open_trades()
    ...
    async def test_get_trade_history_success(self):
        ...
        payload = {"trades": [
            {"id": "t1", "symbol": "ES", "action": "Long",
             "entryPrice": "4500.0", "exitPrice": "4510.0",
             "quantity": "1", "profit": "50.0",
             "openTime": "2025-01-01T09:00:00Z",
             "closeTime": "2025-01-01T10:00:00Z"}
        ]}
        broker.session = _mock_session(200, payload)
...
class TestBrokerApiErrorPaths:
    ...
    async def test_the5ers_place_order_api_error(self):
        from brokers.prop_firms.all_brokers import The5ersBroker
        broker = The5ersBroker("key", "secret", "acc", sandbox=True)
        async def _no_violation(): return (False, None)
        broker.check_risk_violations = _no_violation
        ...
        class MockBroker(BasePropFirmBroker):
            async def get_metrics(self):
                return PropFirmMetrics(
                    ...
                    account_balance=100_000, equity=99_000,
                    used_margin=0, available_margin=99_000,
                    profit_loss=-1_000, profit_loss_percentage=-1.0,
                    daily_drawdown=500, daily_drawdown_percentage=0.5,
                    monthly_drawdown=1_000, monthly_drawdown_percentage=1.0,
                    remaining_days=20, trading_phase=TradingPhase.CHALLENGE,
                    trades_completed=5, win_rate=0.4,
                    largest_win=200, largest_loss=300,
                    consecutive_losses=cl, max_consecutive_losses=mc,
                    account_status=AccountStatus.ACTIVE,
                    daily_loss_limit=5_000, remaining_daily_loss=4_500,
                    monthly_loss_limit=10_000, remaining_monthly_loss=rm,
                )
        ...
        class MockBroker(BasePropFirmBroker):
            async def get_metrics(self):
                return PropFirmMetrics(
                    ...
                    daily_drawdown=5_000, daily_drawdown_percentage=5.5,  # > 5%
                    monthly_drawdown=5_000, monthly_drawdown_percentage=5.0,
                    remaining_days=20, trading_phase=TradingPhase.CHALLENGE,
                    trades_completed=5, win_rate=0.4,
                    largest_win=200, largest_loss=500,
                    consecutive_losses=2, max_consecutive_losses=5,
                    account_status=AccountStatus.ACTIVE,
                    daily_loss_limit=5_000, remaining_daily_loss=100.0,
                    monthly_loss_limit=10_000, remaining_monthly_loss=5_000.0,
                )
            async def place_order(self, *a, **kw): return {}
            async def close_trade(self, trade_id): return {}
            async def get_open_trades(self): return []
