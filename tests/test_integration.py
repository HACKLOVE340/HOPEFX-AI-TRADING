...
def test_kill_switch_activate_deactivate(ks):
    ...
    ks.deactivate(token="test-token-abc")  # nosec B106 - test file
    assert ks.is_active() is False
...
def test_kill_switch_persists_across_instances(tmp_path):
    ...
    ks2.deactivate(token="tok")  # nosec B106 - test file
...
class TestKillSwitchIntegration:
    ...
    def _make_ks(self, tmp_path=None):
        ...
        return KillSwitch(flag_file=flag, deactivation_token="test-token-123")  # nosec B106 - test file
    ...
    def test_deactivate_restores_state(self):
        ks = self._make_ks()
        ks.activate("test")
        ks.deactivate(token="test-token-123")  # nosec B106 - test file
        assert ks.is_active() is False
