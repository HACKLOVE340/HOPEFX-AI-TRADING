...
class DiagnosticsEngine:
    ...
    async def _check_env_vars(self) -> list[DiagnosticResult]:
        ...
        if missing_req:
            results.append(DiagnosticResult(
                check_name="env_vars_required", status="critical",
            ...
            results.append(DiagnosticResult(
                check_name="env_vars_required", status="ok",
                message="All required environment variables are set", duration_ms=dur,
            ))
        if missing_opt:
            results.append(DiagnosticResult(
                check_name="env_vars_optional", status="warning",
    ...
    async def _check_import_chain(self) -> list[DiagnosticResult]:
        ...
        if broken:
            return [DiagnosticResult(
                check_name="import_chain", status="critical",
        ...
        return [DiagnosticResult(
            check_name="import_chain", status="ok",
    ...
    async def _check_database(self) -> DiagnosticResult:
        ...
        try:
            ...
            dur = (time.monotonic() - t0) * 1000
            if ok:
                return DiagnosticResult(check_name="database", status="ok",
                    message="Database connection healthy", duration_ms=dur)
            return DiagnosticResult(check_name="database", status="critical",
                message=f"Database unreachable: {err}",
                remediation="Check DATABASE_URL; ensure DB service is running",
                duration_ms=dur)
        except Exception as exc:
            return DiagnosticResult(check_name="database", status="error",
                ...
                if pong:
                    ...
                    for ns in ["heal:manifest", "alerts:critical", "fixes:approved"]:
                        try:
                            ...
                            ns_status[ns] = False
                    return DiagnosticResult(check_name="redis", status="ok",
                        message="Redis connection healthy",
                        details={"namespace_exists": ns_status}, duration_ms=dur)
                return DiagnosticResult(check_name="redis", status="error",
                    message="Redis PING returned falsy", duration_ms=dur)
            finally:
                await client.aclose()
        except Exception as exc:
            return DiagnosticResult(check_name="redis", status="critical",
    ...
    async def _check_frontend_build(self) -> DiagnosticResult:
        ...
        dur = (time.monotonic() - t0) * 1000
        if found is None:
            return DiagnosticResult(check_name="frontend_build", status="warning",
                message="No frontend build artifact found (index.html missing)",
                details={"searched": [str(p) for p in candidates]},
                remediation="Run: cd frontend && npm run build", duration_ms=dur)
        ...
        if age_hours > _FRONTEND_MAX_AGE_HOURS:
            return DiagnosticResult(check_name="frontend_build", status="warning",
                ...
                remediation="Rebuild frontend: cd frontend && npm run build", duration_ms=dur)
        return DiagnosticResult(check_name="frontend_build", status="ok",
            message=f"Frontend build found and fresh ({age_hours:.1f}h old)",
            details={"path": str(found)}, duration_ms=dur)
    ...
    async def _check_route_health(self) -> list[DiagnosticResult]:
        ...
        try:
            ...
            dur = (time.monotonic() - t0) * 1000
            if unhealthy:
                return [DiagnosticResult(check_name="route_health", status="error",
                    message=f"{len(unhealthy)} route(s) returning 5xx errors",
                    details={"unhealthy": unhealthy, "ok_count": ok_count},
                    remediation="Check application logs for 500 errors", duration_ms=dur)]
            return [DiagnosticResult(check_name="route_health", status="ok",
                message=f"All {ok_count} tested routes are alive", duration_ms=dur)]
        except ImportError:
            return [DiagnosticResult(check_name="route_health", status="warning",
    ...
    async def _check_spa_routing(self) -> list[DiagnosticResult]:
        ...
        try:
            ...
            dur = (time.monotonic() - t0) * 1000
            if broken:
                return [DiagnosticResult(check_name="spa_routing", status="error",
                    ...
                    duration_ms=dur)]
            return [DiagnosticResult(check_name="spa_routing", status="ok",
                message=f"All {ok_count} SPA routes serve HTML correctly", duration_ms=dur)]
        except ImportError:
            return [DiagnosticResult(check_name="spa_routing", status="warning",
    ...
    async def _check_auth_flow(self) -> DiagnosticResult:
        ...
        if not test_email or not test_password:
            return DiagnosticResult(check_name="auth_flow", status="warning",
        ...
        try:
            ...
            async with httpx.AsyncClient(base_url=_APP_BASE_URL, timeout=_DIAG_HTTP_TIMEOUT, follow_redirects=True) as client:
                ...
                if login_resp.status_code != 200:
                    return DiagnosticResult(check_name="auth_flow", status="error",
                ...
                if me_resp.status_code == 200:
                    return DiagnosticResult(check_name="auth_flow", status="ok",
    ...
    async def _check_data_feeds(self) -> list[DiagnosticResult]:
        ...
        try:
            ...
            try:
                ...
                dur = (time.monotonic() - t0) * 1000
                if stale_feeds:
                    results.append(DiagnosticResult(check_name="data_feeds_redis", status="warning",
                        message=f"{len(stale_feeds)} Redis feed key(s) are stale/expired",
                        details={"stale": stale_feeds[:10], "active": found_feeds[:10]},
                        remediation="Check data feed connectors", duration_ms=dur))
                else:
                    results.append(DiagnosticResult(check_name="data_feeds_redis", status="ok",
        ...
        try:
            ...
            results.append(DiagnosticResult(check_name="data_feeds_module",
                status="ok" if ok else "warning",
                message=msg or ("Data feed module healthy" if ok else "Data feed module unhealthy"),
                duration_ms=(time.monotonic() - t0) * 1000))
        except Exception as exc:
            results.append(DiagnosticResult(check_name="data_feeds_module", status="warning",
    ...
    async def _check_log_patterns(self) -> list[DiagnosticResult]:
        ...
        if not log_file.exists():
            return [DiagnosticResult(check_name="log_patterns", status="warning",
        ...
        try:
                    ...
                    for pd in _LOG_PATTERN_MAP:
                        ...
                        if pd["pattern"].search(line):
                            if cat not in findings:
                                findings[cat] = {
                                    "category": cat, "severity": pd["severity"],
                                    "root_cause": pd["root_cause"],
                                    "remediation": pd["remediation"],
                                    "count": 0, "sample": line.strip()[:200],
                                }
            ...
            dur = (time.monotonic() - t0) * 1000
            if not hits:
                return [DiagnosticResult(check_name="log_patterns", status="ok",
                    message="No known error patterns detected in recent logs", duration_ms=dur)]
            results: list[DiagnosticResult] = []
            for hit in hits:
                sev = hit["severity"]
                status = "critical" if sev == "critical" else "error" if sev == "high" else "warning"
                results.append(DiagnosticResult(
                    check_name=f"log_pattern_{hit['category']}", status=status,
                    message=f"[{hit['count']}x] {hit['root_cause']}",
                    details={"count": hit["count"], "sample": hit["sample"]},
                    remediation=hit["remediation"], duration_ms=dur))
            return results
        except Exception as exc:
            return [DiagnosticResult(check_name="log_patterns", status="error",
    ...
    async def _remediate_result(self, result: DiagnosticResult) -> dict[str, Any] | None:
        ...
        action: dict[str, Any] = {
            "check": check, "status": result.status,
        ...
        if action["action"]:
            logger.info("DiagnosticsEngine: remediation %s — action=%s success=%s",
                check, action["action"], action["success"])
            return action
        return None
