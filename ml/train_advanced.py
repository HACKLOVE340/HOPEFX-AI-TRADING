...
def fetch_gold_ohlcv(
    ...
    cached_csv  : Path to cached CSV (default: ``data/XAUUSD_40Y.csv``). Pass
                  ``data/XAUUSD_50Y.csv`` explicitly to opt into the long-history
                  file — the plausibility gate still applies to it.
    """
...
def main(argv: list[str] | None = None):
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Path to cached OHLCV CSV (used with --use-cached; default: data/XAUUSD_40Y.csv). "
            "data/XAUUSD_50Y.csv is not selected automatically — it still passes through the "
            "plausibility gate if named explicitly, which refuses its contaminated windows."
        ),
    )
    parser.add_argument(
        "--smoke",
