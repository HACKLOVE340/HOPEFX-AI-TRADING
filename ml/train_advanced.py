...
def main():
    ...
    logger.info("")
    logger.info("  Walk-forward accuracy : %.3f ± %.3f", wf.get("mean_accuracy", 0), wf.get("std_accuracy", 0))
    logger.info(f"  Walk-forward F1       : {wf.get('mean_f1', 0):.3f}")
    logger.info(f"  Walk-forward AUC      : {wf.get('mean_auc', 0):.3f}")
    logger.info("  p-value (vs random)   : %.4f  %s", wf.get("p_value", 1), "✓ significant" if wf.get("significant") else "✗ not significant")
    logger.info("")
    ...
    if oos_metrics:
        ...
        logger.info("  OOS accuracy          : %.3f ± %.3f  (n=%d)", oos_metrics["accuracy"], acc_se, oos_metrics["oos_size"])
    ...
    if oos_acc >= 0.68:  # noqa: PLR2004
        ...
        logger.info(f"  ⚠ OOS above chance ({oos_acc:.1%}) but below 68% production threshold")
    elif oos_acc > 0:
        ...
        logger.info("  ⚠ Partial in-sample accuracy — run with --oos-years 8 to validate")
    else:
