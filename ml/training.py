    ...
    if ENHANCED_MACRO_AVAILABLE:
        try:
            ...
            if not macro_hist.empty:
                ...
                logger.info("Historical macro features merged: %d series, %d bars", macro_hist.shape[1], len(macro_hist))
            else:
        ...
        try:
            ...
            if macro_features:
                ...
                logger.info("Point-in-time macro features merged (look-ahead bias warning): %s", list(macro_features.keys()))
        except Exception as _macro_exc:
...
def walk_forward_validate(
    ...
    for fold in range(n_splits):
        ...
        logger.info("Fold %d/%d | train=%d test=%d | accuracy=%.3f | f1=%.3f",
                     fold + 1, n_splits, len(X_train), len(X_test),
                     metrics.get("accuracy", 0), metrics.get("f1", 0))
    ...
    logger.info("Walk-forward summary (%d folds): accuracy=%.3f ± %.3f | f1=%.3f ± %.3f",
                 len(fold_results), summary["mean_accuracy"], summary["std_accuracy"],
