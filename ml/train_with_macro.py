...
def main():
    ...
    for model_type in ("xgb", "rf"):
        ...
        logger.info(f"\n{model_type.upper()}")
        logger.info("  Walk-forward accuracy : %.3f ± %.3f", wf["mean_accuracy"], wf["std_accuracy"])
        logger.info(f"  Walk-forward F1       : {wf['mean_f1']:.3f}")
        mean_sharpe = wf.get("mean_sharpe", 0.0)
        logger.info("  Walk-forward Sharpe   : %.3f  (annualised, 1-bar, no costs — N < 250: SE ≈ ±0.54)", mean_sharpe)
        logger.info("  p-value (vs random)   : %.4f  %s", wf["p_value"], "✓ significant" if wf["significant"] else "✗ not significant")
        ...
        if f"oos_{model_type}" in report:
            ...
            logger.info("  OOS period            : %s", oos.get("oos_period", "n/a"))
            logger.info("  OOS accuracy          : %.3f ± %.3f  (n=%d)", oos["accuracy"], acc_se, oos["oos_size"])
