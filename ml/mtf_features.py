...
def build_mtf_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    GOLD_SEASONAL_SCORE = {
        1: 0.7, 2: 0.3, 3: -0.4, 4: -0.5, 5: 0.1, 6: 0.2,
        7: 0.3, 8: 0.8, 9: 0.9, 10: 0.4, 11: 0.2, 12: 0.5,
    }
    ...
    d["mtf_recovering"]     = (
        (d["mtf_drawdown_252"] > -0.10) &
        (d["mtf_drawdown_252"].shift(20) < -0.10)
    ).astype(int)
    ...
    bull_signals = [
        "mtf_w_bull", "mtf_m_bull", "mtf_q_bull",
        "mtf_w_rsi_bull", "mtf_m_rsi_bull",
        "mtf_m_bull_12m", "mtf_q_bull",
    ]
    bear_signals = [
        "mtf_w_rsi_ob", "mtf_m_rsi_ob",
        "mtf_m_bear_12m", "mtf_in_drawdown",
    ]
...
def _hurst_proxy(s: pd.Series, n: int = 20) -> pd.Series:
    ...
    hurst = returns.rolling(n).apply(
        lambda x: 0.5 + 0.5 * np.tanh(np.log(_rs(x) + 1e-8) / np.log(n) - 0.5)
        if len(x) >= 4 else 0.5,
        raw=True,
    ).fillna(0.5)
    return hurst
