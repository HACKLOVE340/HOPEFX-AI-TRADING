...
|---|---|
| **How does this branch reach `main`?** | `docs/audit/LANDING_PLAN.md` — 623 commits and 1,525 files ahead, cut into nine reviewable slices with a proven recipe. Read it before opening a pull request. |
...
| Why does changing the density barely change anything? | Because it reaches 29 places. `data-density` is real and proven live in a browser (gap 16→10→7px, body 15→13→12px, card padding 20→14→10px), but 2,871 inline `fontSize: <number>` and 1,089 numeric spacing utilities are literal pixels the cascade cannot touch. Same defect as the colour literals, in the size dimension. `python scripts/correction_register.py --id DENSITY-CANNOT-REACH` |
