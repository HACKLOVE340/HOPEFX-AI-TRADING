...
|---|---|
| **How does this branch reach `main`?** | `docs/audit/LANDING_PLAN.md` — 631 commits and 1,535 files ahead, cut into nine reviewable slices with a proven recipe. Read it before opening a pull request. |
...
| Why do the terminals show flat candles, and nothing at 1h? | Two separate things, both diagnosed 2026-09-15 and neither in the chart code. **Flat candles:** the bars really are flat — measured on the daily series the platform serves offline, 62 of 500 have no body or no wicks. `frontend/src/lib/barQuality.ts` now says so on the chart instead of drawing them silently. **Nothing at 1h:** XAUUSD has no intraday source without a live feed — its yfinance ticker is deliberately empty (Yahoo delisted it, verified 2026-07-26; a previous hand-maintained copy served delisted `GC=F` futures as spot) and the bundled CSV is daily/weekly only. The endpoint correctly 503s with the feed to configure rather than fabricating. Do not put a yfinance ticker back. |
