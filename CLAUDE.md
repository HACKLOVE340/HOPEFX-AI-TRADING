...
| Why are there still emoji where icons belong? | `python scripts/frontend_emoji_ratchet.py --check` · `docs/FRONTEND_EMOJI_DEBT.json` — **6 across 4 files**, down from 1,407 at the audit, from 750 on 2026-09-18, and from 58 on 2026-09-24 (F175). Retire them with `python scripts/frontend_emoji_codemod.py --check` · `--apply`, which converts ONLY a glyph outside every string literal in a `.tsx` file — a bare glyph there can only be JSX children, and converting one inside a string yields `'<Zap /> Plan'` rendered to the user. It emits `size="1em"`, never a pixel count, so the icon follows `data-density` exactly as the glyph did and a 32px empty-state glyph needs no per-site decision. **The 6 left are the ones it refuses, and each was left for a specific reason:** `useNuclearWS.ts` stays at 1 on purpose — the gate is a RANGE SWEEP, not a presentation test, and it cannot express a glyph in a string the browser renders itself. `AdminPanel.tsx` and `superadmin/AutoHealingSection.tsx` each hold one glyph inside a plain-`string`-typed prop (`KpiCard`'s `sub`, `showMsg`'s `text`) that cannot carry JSX without widening the prop's type — a per-site decision, not a codemod's. `Affiliate.tsx` holds 3 medal glyphs (🥇🥈🥉) in a leaderboard-rank lookup with no unambiguous Lucide equivalent that preserves the 1st/2nd/3rd distinction. Everywhere else, the `label: '<glyph> Text'` arrays and inline ternaries were converted: an entry gains an `icon:` field and the render site draws it, which is a per-array change no codemod can make. Pick a name that shadows nothing — `MapIcon` not `Map` (`new Map()` is 1,100 lines below the import in PlatformConfiguration), `Link2` not `Link` (react-router exports one); regional-indicator flags (Lucide has none; on Windows the pair already renders as letters, so a country code is the honest replacement), faces used as a sentiment scale, and comments, which want rewording. **Converting one can DELETE an accessible name:** six controls had theirs only because the glyph was their text content. `npx vitest run src/test/controls_use_icons_not_glyphs.test.tsx` fails if an icon-only button has no name |
