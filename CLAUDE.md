...
| Why are there still emoji where icons belong? | There are none — **0 across 0 files**, down from 1,407 at the audit, 750 on 2026-09-18, 58 on 2026-09-24 and the last 6 closed the same day. `python scripts/frontend_emoji_ratchet.py --check` · `docs/FRONTEND_EMOJI_DEBT.json` (an empty `files: {}` map — the baseline entry is deleted the moment a file reaches zero, per the ratchet's own rule, and the last four to reach it did so together). **F175 is closed.** The final 6, each deliberate rather than a codemod miss: the `document.title` glyph in `useNuclearWS.ts` is now plain text (`[!] NUCLEAR ALERT`) — a browser tab title has no markup, so no SVG could ever have replaced it, but the owner decided the deliberate exemption should end rather than stand forever; the `sub`/`text` props on `AdminPanel.tsx`'s `KpiCard` and `superadmin/AutoHealingSection.tsx`'s `showMsg` are widened from `string` to `React.ReactNode`, and each call site now renders a Lucide icon (`AlertTriangle`, `XCircle`) at `size="1em"` with `aria-hidden`; the three medals in `Affiliate.tsx` are a `RankBadge` — a Lucide `Medal` tinted by the new `--rank-gold` / `--rank-silver` / `--rank-bronze` tokens (`frontend/src/index.css`, both themes) alongside the numeral and an `aria-label` ("1st place"), so rank is never colour-only. Retire a future one with `python scripts/frontend_emoji_codemod.py --check` · `--apply`, which converts ONLY a glyph outside every string literal in a `.tsx` file — a bare glyph there can only be JSX children, and converting one inside a string yields `'<Zap /> Plan'` rendered to the user. It emits `size="1em"`, never a pixel count, so the icon follows `data-density` exactly as the glyph did and a 32px empty-state glyph needs no per-site decision. The gate is a RANGE SWEEP, not a presentation test — it counts `✓ ✕ ✗`, which are text-presentation dingbats in the icon role — and it cannot express a glyph in a string the browser renders itself, which is why the `useNuclearWS.ts` case needed a hand rewrite rather than a codemod run. **Converting one can DELETE an accessible name:** six controls had theirs only because the glyph was their text content. `npx vitest run src/test/controls_use_icons_not_glyphs.test.tsx` fails if an icon-only button has no name. The `check()` rule that a baseline file holding a genuinely empty `files: {}` map is not "no baseline" — it used to read as one, refusing a fully-clean tree with "run --adopt" after `--adopt` had already been run — is pinned by `test_reaching_zero_across_every_file_still_passes` in `tests/unit/test_frontend_emoji_ratchet.py` |
