...
|---|---|
| **How does this branch reach `main`?** | `docs/audit/LANDING_PLAN.md` — 621 commits and 1,512 files ahead, cut into nine reviewable slices with a proven recipe. Read it before opening a pull request. |
...
| Why is the head a wireframe skull and not an ellipse? | `frontend/src/hub/head.ts::headMesh` — a volumetric mesh that turns toward what it is looking at and hinges its jaw on the character being spoken. The old head was an outline, and an outline has no orientation. `npx vitest run src/test/the_head_has_a_skull.test.ts` |
