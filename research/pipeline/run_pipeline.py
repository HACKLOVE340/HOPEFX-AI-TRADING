...
def _print_report(report: dict, ticker: str) -> None:
    ...
    logger.info(f"\n{'─' * 60}")
    logger.info(f"  {ticker}  |  run_id={report['run_id']}")
    logger.info(f"{'─' * 60}")
    logger.info(f"  AUC          : {test.get('auc', 0):.4f}")
    logger.info(f"  Accuracy     : {test.get('accuracy', 0):.4f}")
    logger.info(f"  F1           : {test.get('f1', 0):.4f}")
    logger.info(f"  Precision    : {test.get('precision', 0):.4f}")
    logger.info(f"  Recall       : {test.get('recall', 0):.4f}")
    logger.info(f"  Sharpe       : {test.get('strategy_sharpe', 0):.2f}")
    logger.info(f"  Max Drawdown : {test.get('strategy_max_drawdown', 0) * 100:.1f}%")
    logger.info(f"  Total Return : {test.get('strategy_total_return', 0) * 100:.1f}%")
    logger.info(f"  Hit Rate     : {test.get('hit_rate', 0):.4f}")
    logger.info(f"  Meta-weight  : deep={report['meta_weight']:.2f}  ens={1 - report['meta_weight']:.2f}")
    logger.info(f"  Artefacts    : {report['artefact_dir']}")
    logger.info(f"{'─' * 60}\n")
