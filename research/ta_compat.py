...
class EMAIndicator:
    ...
    def ema_indicator(self) -> pd.Series:
        if _TALIB_OK:
            ...
            arr = (
                self._close.ewm(span=self._window, adjust=False).mean().values
            )  # healer: ignore — NaN for warmup bars is expected TA behaviour; callers use _wrap
        return _wrap(arr, self._close.index)
...
def _atr_numpy(high, low, close, period=14) -> np.ndarray:
    ...
    return (
        tr.ewm(com=period - 1, min_periods=period).mean().values
    )  # healer: ignore — NaN for warmup bars is expected TA behaviour
...
def _adx_numpy(high, low, close, period=14) -> np.ndarray:
    ...
    pdi = (
        100 * plus_dm.ewm(com=period - 1, min_periods=period).mean() / (atr + 1e-12)
    )  # healer: ignore — NaN for warmup bars is expected TA behaviour
    mdi = (
        100 * minus_dm.ewm(com=period - 1, min_periods=period).mean() / (atr + 1e-12)
    )  # healer: ignore — NaN for warmup bars is expected TA behaviour
    dx = 100 * (pdi - mdi).abs() / (pdi + mdi + 1e-12)
    return (
        dx.ewm(com=period - 1, min_periods=period).mean().values
    )  # healer: ignore — NaN for warmup bars is expected TA behaviour
