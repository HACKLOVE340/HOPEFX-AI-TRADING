...
# ════════════════════════════════════════════════════════════════════════════════
def _coverage(covered: int, total: int, rule: str, what: str, min_fraction: float = 1.0,
              severity: str = CRITICAL) -> list[Violation]:
    if total <= 0:
        return []
    fraction = covered / total
    if fraction < min_fraction:
        return [_v(rule, severity,
                   f"{what} coverage {covered}/{total} ({fraction:.0%}) below required {min_fraction:.0%}",
                   covered=covered, total=total)]
    return []


def verify_invariant_coverage(protected_components: int, total_components: int,
                              min_fraction: float = 1.0) -> list[Violation]:
    """Every critical component must have invariants guarding it."""
    return _coverage(protected_components, total_components, "No Silent Failure",
                     "invariant", min_fraction)


def verify_alert_coverage(failures_with_alerts: int, total_critical_failures: int,
                          min_fraction: float = 1.0) -> list[Violation]:
    """Every critical failure mode must generate an alert."""
    return _coverage(failures_with_alerts, total_critical_failures, "No Silent Failure",
                     "alert", min_fraction)


def verify_recovery_coverage(services_with_recovery: int, total_critical_services: int,
                             min_fraction: float = 1.0) -> list[Violation]:
    """Every critical service must have a recovery procedure."""
    return _coverage(services_with_recovery, total_critical_services, "No Unrecoverable Failure",
                     "recovery", min_fraction)


def verify_ai_explainability_coverage(explained_decisions: int, total_decisions: int,
                                      min_fraction: float = 1.0) -> list[Violation]:
    """Every AI decision must carry an explanation."""
    return _coverage(explained_decisions, total_decisions, "No Unverified AI Decision",
                     "AI-explainability", min_fraction)


def verify_decision_trace(trace: dict[str, Any],
                          links: tuple[str, ...] = (
                              "decision_id", "model_version", "prompt_hash",
                              "features_hash", "data_snapshot", "trade_id")) -> list[Violation]:
    ...
    if missing:
        return [_v("No Hidden AI Action", CONSTITUTIONAL,
                   f"decision trace not reconstructable — missing link(s): {missing}", missing=missing)]
    return []
