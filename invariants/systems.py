...
def verify_quorum(healthy_nodes: int, total_nodes: int) -> list[Violation]:
    ...
    if total_nodes > 0 and healthy_nodes <= total_nodes // 2:
        return [_v("No Critical Single Point Of Failure", CRITICAL,
                   f"quorum lost: {healthy_nodes}/{total_nodes} healthy")]
    return []
...
def verify_acyclic(graph: Mapping[Any, Iterable[Any]], rule: str = "No Unbounded Failure",
                   what: str = "dependency") -> list[Violation]:
...
def verify_no_zombies(count: int, what: str) -> list[Violation]:
    ...
    if count > 0:
        ...
        return [_v("No Hidden Exposure" if what in ("capital", "position") else "No Silent Failure", sev,
                   f"{count} zombie {what}(s) detected")]
    return []


def verify_blast_radius_contained(failed_components: int, total_components: int,
                                  max_fraction: float = 0.5) -> list[Violation]:
