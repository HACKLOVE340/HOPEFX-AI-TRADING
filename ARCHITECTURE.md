...
| `python scripts/crypto_address_collision_report.py` | Read-only: which stored crypto deposit addresses (`crypto_payments` table and the billing order store) were issued to more than one payment/order — the BTC-index-0 and ETH/USDT_ERC20 derivation collisions fixed 2026-09-25 (MASTER_OUTSTANDING §A11). Never touches the network; matching an on-chain deposit to a listed payment by amount and time is a manual owner step |
...
| `python scripts/frontend_emoji_codemod.py --check` · `--apply` | Converts an emoji used as an icon to the Lucide element that means it, at `size="1em"` so it follows `data-density`. Only outside string literals, in `.tsx` |
