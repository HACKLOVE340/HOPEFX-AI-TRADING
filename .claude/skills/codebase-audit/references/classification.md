...
For security-specific findings, use the detailed checklist in `references/security-severity-checklist.md`. It maps each vulnerability type to a severity tier with concrete technical conditions, and opens with a trading-system escalation rule that overrides the generic tiers:

| Tier | Audit Severity | Example Vulnerabilities |
|------------|---------------|----------------------|
| Critical | **Critical** | RCE with server access, broker-credential leak, risk-gate or kill-switch bypass, unauthenticated order path |
| High | **High** | Stored XSS (authenticated dashboard), SQL injection (readable), unauthorized admin, SSRF w/ response |
| Medium | **Medium** | Stored XSS (needs interaction), blind SSRF, local code exec, single-API IDOR |
| Low | **Low** | XSS in obsolete browsers, open redirect, rate limiting gaps |
| 无 (None) | **Ignore** | Self-XSS, no-impact CSRF, scanner noise |

When classifying a security finding, first assign its tier using the condition-based checklist in `security-severity-checklist.md`, then map to the audit severity above.
