...
Severity checklist for classifying security findings during codebase audits.
Each vulnerability type is mapped to a severity tier with concrete technical
conditions. Adapted for HOPEFX: a money-moving trading system where the assets
at risk are broker credentials, order flow, and the risk controls themselves.

## Severity Mapping

| Tier | Audit | Action |
|------|-------|--------|
| Critical | **Critical** | Immediate fix, individual batch |
| High | **High** | Fix in current session, batch by type |
| Medium | **Medium** | Fix if time allows |
| Low | **Low** | Defer or batch |
| None | **Ignore** | Record as informational, no action |

## Trading-system escalation rule

Before applying the generic tiers below, check whether the finding touches any
of these. If it does, it is **Critical** regardless of what the generic tier
would say, and it is **never** auto-fixed into a PR without human review:

- [ ] The pre-trade risk gate, VaR/CVaR, Kelly sizing, or exposure limits (`risk/manager.py`)
- [ ] The kill switch — its trip condition, its reset path, or anything that can bypass it
- [ ] Model staleness or drift gating in `ml/inference_engine.py`
- [ ] Order construction, submission, or reconciliation in `execution/oms.py` or `brokers/`
- [ ] Broker or exchange credential handling, storage, or logging
- [ ] The 5-phase decision pipeline in `core/decision/HOPEFXDecisionEngine.py`
- [ ] Prop-firm limit enforcement (`prop_firm_mode.json` ruleset application)

A change that makes any of these *more permissive* is a finding in itself, even
if the code is otherwise correct.
...
### C2 — Mass Information Leakage

**Conditions (all must be met):**
- [ ] Leaked information can compromise account or trading security
- [ ] No significant barriers to exploitation

Checklist:
- [ ] Broker API keys, OANDA tokens, or MT5 credentials readable without auth
- [ ] Bulk user identity or account data accessible without authentication
- [ ] API endpoint returns full user profiles or positions without auth checks
- [ ] Credentials or tokens written to logs, tracebacks, or error responses
- [ ] Secrets committed to the repository rather than read from the environment
...
### C4 — Direct Capital Loss

**Conditions (any one is sufficient):**
- [ ] An attacker or a bug can cause real orders to be placed, sized, or cancelled
- [ ] A risk limit can be bypassed, disabled, or silently widened
- [ ] Account balance or position state can be manipulated

Checklist:
- [ ] Unauthenticated or under-authenticated order-submission endpoint
- [ ] Risk gate can be skipped by input shape, flag, or exception path
- [ ] Kill switch can be reset or suppressed without authorization
- [ ] Position size derived from unvalidated external input
- [ ] Billing, subscription, or licence logic bypass

---

## HIGH

### H1 — Stored XSS (Authenticated Dashboard)

For the trading dashboard and any authenticated operator surface, where a
successful XSS can drive order actions with the operator's session.
...
**Note:** When several routes share one frontend template, report the shared
sink once rather than filing one finding per route.
...
**Conditions:**
- [ ] Can target internal infrastructure (broker VPN, database, Redis, metrics) rather than only public endpoints
...
### H11 — Entitlement / Quota Drain

**Conditions (all must be met):**
- [ ] A user can grant themselves paid entitlements — licence tiers, subscription
      plans, API quota, backtest compute — outside intended behaviour
- [ ] The gain is material rather than incidental

Checklist:
- [ ] Licence or subscription tier settable from client input
- [ ] Rate limit or quota counter resettable by the caller
- [ ] Trial or promotional grant replayable without limit
...
- [ ] A project-controlled subdomain is vulnerable to takeover
...
- [ ] Can reach internal infrastructure but no response is returned
...
### M8 — Scoped Integration Key Leak (Limited Impact)

- [ ] A third-party integration key with limited scope is leaked
- [ ] Cannot prove exploitation of high-impact APIs (order placement, credential rotation, any write path)
...
- [ ] Cannot reach the production or internal network
...
- [ ] Project-controlled domains and subdomains

**Proof requirement:**
- [ ] Must demonstrate a direct redirect to an attacker-controlled origin
...
- [ ] Not a vulnerability in this project's own code or deployment
- [ ] Not a bug in a third-party product this project merely consumes

### N6 — Under Active Remediation

- [ ] The affected system is already under an open remediation ticket
- [ ] Check the existing issue tracker for a duplicate before filing
...
│   ├── Data exfiltration?
│   │   ├── Broker credentials or bulk account data? → CRITICAL
...
│   ├── Direct capital loss?
│   │   ├── Can place/size/cancel real orders, or bypass a risk limit? → CRITICAL
│   │   ├── Entitlement or quota drain? → HIGH
│   │   └── Paper-trading only, no real capital path? → MEDIUM
│   └── DoS?
...
3. **Record** the tier as an annotation (e.g., `[SEV: HIGH]`) alongside the audit severity
4. **Assign** the mapped audit severity for the fix batch priority

For findings where conditions are ambiguous, default to the lower severity and
note the uncertainty — **except** for anything caught by the trading-system
escalation rule at the top of this file, where ambiguity escalates rather than
de-escalates.
