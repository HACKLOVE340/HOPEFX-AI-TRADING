...
description: Use when running or scheduling periodic repository review cycles that must dispatch the correct reviewer, aggregate findings, and escalate outcomes into reports, issues, or corrective PRs across code quality, security, documentation freshness, open PRs, and skill quality.
alwaysApply: false
---
...
- Replacing the existing skill review flow under `skill-authoring`
...
- Published docs and agent-contract drift (`CLAUDE.md`, `AGENTS.md`, `ARCHITECTURE.md`, `docs/`) → `doc-freshness-review`
- Broad code hygiene and proactive repository health → `codebase-audit`
- Existing open PR triage and repair → `pr-review-fix`
- Skill quality under `.claude/skills` → `skill-authoring`, then load its `repo-skill-review` reference
- Security posture of a specific change set → the built-in `/security-review`
- Correctness review of the current diff or a named PR → the built-in `/code-review`

Do not create a redundant top-level reviewer when an existing flow already covers the surface. In particular, do not reimplement diff review — dispatch to `/code-review`.

This repository moves money. Any finding that touches a risk gate, kill switch, staleness or drift check, or order-execution path is **escalate-to-human by default**, whatever its mechanical severity: report it, never auto-fix it into a PR.
...
3. Keep reports readable: separate code, security, docs, PR, and skill findings.
...
| Review skills under `.claude/skills` | `skill-authoring` |

## Evaluation prompts

### Should-trigger

1. Run a weekly repository review that checks code health, docs freshness, and skill quality, then decide what should become PRs.
...
3. Aggregate findings from code, docs, and skill review into one actionable maintenance report.

### Should-not-trigger

1. Review this one MCP tool for parameter casing errors.
2. Audit `doc/` for stale links and missing files.
3. Rewrite a single skill description to improve its trigger wording.

## Minimum self-check

- Did I dispatch each finding class to the correct specialized reviewer?
- Did I reuse `skill-authoring` for skill review instead of inventing a duplicate flow?
