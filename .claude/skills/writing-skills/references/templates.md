...
## New skill landing checklist

When adding a skill to this repository:

1. Write `.claude/skills/<dir>/SKILL.md`. The frontmatter `name` must equal `<dir>`.
2. Keep supporting material in `.claude/skills/<dir>/references/` and `.claude/skills/<dir>/scripts/`.
3. **Required:** record provenance and licence in `.claude/skills/README.md` if the skill is vendored from another project.
   - Note upstream repo, pinned commit, licence, and any local patches.
   - Re-verify any bundled script still runs after a version bump.
