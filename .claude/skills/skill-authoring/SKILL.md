...
- Audit a local skill collection such as `.claude/skills` for redundancy, trigger overlap, or weak boundaries
...
## Repo-managed skill review

When the task targets `.claude/skills`, apply these guardrails in addition to the normal skill-authoring workflow:

This section is the repo-managed skill review baseline for this repository.
...
- Human documentation URLs remain allowed when they are not skill-body fetch instructions.
...
5. **Use collection-level review when the request is about many skills**
   - When reviewing `.claude/skills`, check overlap, duplication, trigger boundaries, and progressive disclosure across neighboring skills
...
| Audit `.claude/skills` for quality, redundancy, and overlap | `references/repo-skill-review.md` |
...
3. If the task targets `.claude/skills`, read `references/repo-skill-review.md` for the collection-review baseline before proposing rewrites.
...
7. Place the skill under `.claude/skills/<dir>/SKILL.md` so Claude Code discovers it automatically. Keep supporting material in `references/` and `scripts/` beside it.
...
- Does the skill live at `.claude/skills/<dir>/SKILL.md` with a `name` matching its directory name?
