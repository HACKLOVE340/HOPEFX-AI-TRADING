...
class RevenueAnalytics:
    ...
    def get_top_customers(
        ...
        sorted_users = sorted(user_revenue.items(), key=lambda x: x[1], reverse=True)

        return [
            {"rank": idx + 1, "user_id": user_id, "total_revenue": float(amount)}
    ...
    def get_cohort_analysis(self, cohort_period: TimePeriod = TimePeriod.MONTHLY) -> dict[str, Any]:
        ...
        return {"cohort_period": cohort_period.value, "cohorts": cohort_retention}
