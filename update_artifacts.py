...
def generate_cover(output: Path = OUTPUT_PATH) -> Path:
    ...
    for i, (label, value, note) in enumerate(cards):
        ...
        draw.rounded_rectangle(
            ...
            radius=10, fill=_SHADOW,
        )
        # Card body
        draw.rounded_rectangle(
            [(cx, card_y), (cx + card_w, card_y + card_h)],
            radius=10, fill=_CARD_BG, outline=_CARD_BORDER, width=1,
        )
        # Top gold strip
        draw.rounded_rectangle(
            [(cx + 1, card_y + 1), (cx + card_w - 1, card_y + 4)],
            radius=2, fill=_GOLD_DIM,
        )
    ...
    draw.rounded_rectangle(
        ...
        radius=15, fill=_PILL_BG, outline=(28, 96, 48), width=1,
    )
    ...
    for text, bg in badges:
        ...
        draw.rounded_rectangle(
            ...
            radius=15, fill=bg, outline=(70, 70, 110), width=1,
        )
