...
def print_report(ledger: dict[str, Any]) -> None:
    ...
    print()
    print("=" * 60)
    print("  HOPEFX Paper Trading Fill Gate")
    print("=" * 60)
    print(f"  Fills recorded : {fill_count:,}")
    print(f"  Gate target    : {FILL_GATE_TARGET:,}")
    print(f"  Progress       : {pct:.1f}%  [{fill_count}/{FILL_GATE_TARGET}]")
    print(f"  Remaining      : {remaining:,}")
    print(f"  Gate status    : {'PASSED' if gate_passed else 'NOT YET PASSED'}")
    if ledger.get("first_fill_at"):
        print(f"  First fill     : {ledger['first_fill_at']}")
    if ledger["fills"]:
        print(f"  Last fill      : {ledger['fills'][-1]['time']}")
    print(f"  Last sync      : {ledger.get('last_sync_at', 'never')}")
    print("=" * 60)

    if gate_passed:
        print()
        print("  Gate PASSED. Phase 3 (live capital) deployment is unblocked")
        print("  by the fill count requirement. Verify remaining checklist:")
        print("  - 30+ days continuous paper trading without system errors")
        print("  - Monte Carlo backtest reconciled with OOS evaluation")
        print("  - Risk committee sign-off")
    else:
        print()
        print(f"  Gate NOT PASSED. Need {remaining:,} more fills before Phase 3.")
        print("  Run paper_trading_starter.py to accumulate fills.")
    print()
