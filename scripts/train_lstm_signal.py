...
def train(args: argparse.Namespace) -> None:
    ...
    logger.info("  Val accuracy : %.4f", accuracy)
    logger.info("  Features     : %s", n_features)
    logger.info("  Seq len      : %s", args.seq_len)
    logger.info("  Saved to     : %s", out_path)
    logger.info("\nNext steps:")
    logger.info("  1. Add to .env:")
    logger.info("       LSTM_MODEL_PATH=%s", out_path)
    logger.info("       LSTM_SIGNAL_WEIGHT=0.2")
    logger.info("  2. Restart the app — LSTM will blend at 20% weight")
    logger.info("=" * 60)
