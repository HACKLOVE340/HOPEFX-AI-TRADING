...
def dry_run(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        logger.info("  Horizon      : %s bars", args.horizon)
        logger.info("  OHLCV bars   : %s", len(ohlcv))
        logger.info("  Feature rows : %s", len(X))
        logger.info("  Feature cols : %s", X.shape[1])
        logger.info("  Class balance: %s", y.value_counts().to_dict())
        logger.info("=" * 60)
        sys.exit(0)
...
def print_horizon_summary(args: argparse.Namespace, report: dict) -> None:
    ...
    logger.info("=" * 65)
    logger.info("  Training horizon : %s bars  (was: 1 bar)", args.horizon)
    logger.info("  Hold period      : %s bars  (execution engine)", args.horizon)
    logger.info("  Horizon aligned  : YES — accuracy/P&L now measure same objective")
    logger.info("")
    logger.info("  Years of data    : %s", args.years)
    logger.info("  OOS period       : %.1f years", args.oos_years)
    logger.info("  Features         : %s", report.get("feature_count", "?"))
    logger.info("  Samples          : %s", report.get("sample_count", "?"))
    logger.info("")
    if wf:
        logger.info("  Walk-forward acc : %.3f ± %.3f", wf.get("mean_accuracy", 0), wf.get("std_accuracy", 0))
        logger.info("  Walk-forward F1  : %.3f", wf.get("mean_f1", 0))
    if oos:
        sig = "✓ significant" if oos.get("significant") else "✗ not significant"
        logger.info("  OOS accuracy     : %.3f  %s", oos.get("accuracy", 0), sig)
