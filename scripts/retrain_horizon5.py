...
def print_horizon_summary(args: argparse.Namespace, report: dict) -> None:
    ...
    print("\n" + "=" * 65)
    print("HORIZON-5 RETRAIN SUMMARY")
    print("=" * 65)
    ...
    print(f"  Features         : {report.get('feature_count', '?')}")
    ...
    if oos:
        ...
        print(f"  OOS accuracy     : {oos.get('accuracy', 0):.3f}  {sig}")
        print(f"  OOS F1           : {oos.get('f1', 0):.3f}")
        print(f"  OOS p-value      : {oos.get('p_value_binomial', 1):.4f}")
    print()
    print("  Model saved → ml/saved_models/advanced_oos.pkl")
    print("  Meta  saved → ml/saved_models/horizon5_meta.json")
    print()
    print("  Next steps:")
    print("  1. Restart the inference engine (or it will auto-reload on next predict)")
    print("  2. Monitor /api/ml/health — last_trained_at should update")
    print("  3. Run 30-day paper trading to validate P&L improvement")
    print("  4. Compare OOS accuracy vs previous horizon=1 model (66.35%)")
    print("=" * 65)
