...
async def main() -> None:
    ...
    logger.info("\n" + "=" * 60)
    logger.info(f"TICK BACKTEST RESULTS — {args.symbol}  {start.date()} → {end.date()}")
    logger.info("=" * 60)
    logger.info(f"  Bars analysed  : {len(ohlcv)}")
    logger.info(f"  Signals fired  : {results.get('signals', 0)}")
    ...
    logger.info(f"  Final equity   : ${results.get('final_equity', args.capital):,.2f}")
    logger.info(f"  Max drawdown   : {results['max_drawdown']:.1%}")
    logger.info(f"  Sharpe (ann.)  : {results['sharpe']:.3f}")
