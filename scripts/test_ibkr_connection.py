    ...
    try:
        ...
        try:
            ...
            failures += int(not _check(
                ...
                has_price,
                f"last={ticker.last} close={ticker.close}" if has_price
                else "no price data — check market data subscriptions"
            ))
            ib.cancelMktData(contract)
        except Exception as md_exc:
            failures += int(not _check(
                "Market data snapshot (GC/XAUUSD)", False, str(md_exc)
            ))
        ...
        if PORT in PAPER_PORTS and interactive:
            try:
                ...
                failures += int(not _check(
                    ...
                    if ok else "whatIfOrder returned None"
                ))
            except Exception as ord_exc:
                failures += int(not _check(
                    "Order dry-run (whatIfOrder)", False, str(ord_exc)
                ))
