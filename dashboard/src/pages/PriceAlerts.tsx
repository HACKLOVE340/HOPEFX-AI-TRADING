      ...
      <div style={s.tabs}>
        {(['active', 'history'] as const).map((t) => (
          <button key={t} onClick={() => setTab(t)}
            ...
            {t === 'active' ? `Active Alerts (${alerts.filter(a => a.status !== 'cancelled').length})` : 'Trigger History'}
          </button>
        ))}
      </div>
      ...
      {tab === 'active' && (
        ...
        alerts.map((alert) => (
          <div key={alert.id} style={s.alertRow}>
            ...
            <button onClick={() => handleToggle(alert)} style={s.iconBtn} title={alert.status === 'paused' ? 'Resume' : 'Pause'}>
              {alert.status === 'paused' ? '▶' : '⏸'}
            </button>
            <button onClick={() => handleDelete(alert.id)} style={{ ...s.iconBtn, color: '#f87171' }} title="Delete">
              🗑
            </button>
          </div>
        ))
        ...
        history.slice(0, 50).map((t, i) => (
          <div key={i} style={s.historyRow}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <span style={{ color: '#64748b', fontSize: 12, marginLeft: 8 }}>{t.symbol} @ {t.trigger_value}</span>
            </div>
...
const s: Record<string, React.CSSProperties> = {
  ...
  channelBtnActive: { background: '#1e3a5f', borderColor: '#3b82f6', color: '#60a5fa' },
