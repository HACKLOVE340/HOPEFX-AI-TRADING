...
@cuda.jit
def _cuda_macd_kernel(
    prices, fast_period, slow_period, signal_period, out_macd, out_signal, out_hist
):
...
class FeatureEngineer:
    ...
    def __init__(
        ...
        self._hawkes_intensity = (
            torch.tensor(0.0, device="cuda") if self.use_gpu else 0.0
        )
    ...
    def _init_gpu_advanced(self) -> None:
        ...
        try:
            with nvtx.range("init_gpu"):
                ...
                # Memory pool
                self._memory_pool = CUDAMemoryPool(
                    max_tensors=50, tensor_size=self._buffer_capacity
                )
                ...
                self._unified_buffer = torch.empty(
                    (
                        ...
                        6,
                    ),  # mid, volume, bid, ask, timestamp, spread
                    dtype=torch.float32,
                    device="cuda",
                )
                ...
                self._gpu_indicators = torch.empty(
                    (
                        self._buffer_capacity,
                        10,
                    ),  # rsi, atr, macd, signal, hist, obv, vwap, etc
                    dtype=torch.float32,
                    device="cuda",
                )

                # Hawkes state
                self._hawkes_times = torch.empty(
                    1000, dtype=torch.float32, device="cuda"
                )

                # Persistent indicator state
                self._gpu_rsi_state = torch.zeros(
                    2, device="cuda"
                )  # avg_gain, avg_loss
    ...
    async def _gpu_pipeline(self, latest_tick: Tick) -> None:
        ...
        with nvtx.range("gpu_pipeline"):
            ...
            with torch.cuda.stream(self._streams[0]):
                prices = torch.tensor(
                    [float(t.mid) for t in self._ticks_buffer[-200:]], device="cuda"
                )
            ...
            with torch.cuda.stream(self._streams[2]):
                volumes = torch.tensor(
                    [float(t.volume) for t in self._ticks_buffer[-100:]], device="cuda"
                )
            ...
            with torch.cuda.stream(self._streams[3]):
                self._gpu_hawkes = self._cuda_hawkes(
                    torch.tensor(
                        float(latest_tick.timestamp.timestamp()), device="cuda"
                    ),
                    self._hawkes_decay,
                    self._hawkes_intensity,
                )
    ...
    def _cuda_hawkes(
        self, timestamp: Tensor, decay: Tensor, intensity: Tensor
    ) -> Tensor:
        """GPU Hawkes process."""
        ...
        last_time = (
            self._hawkes_times[0] if self._hawkes_times.numel() > 0 else timestamp - 1.0
        )
    ...
    async def _compute_hybrid(self, tick: Tick) -> FeatureVector:
        ...
        if self.use_gpu and hasattr(self, "_gpu_rsi"):
            ...
            macd_signal = (
                float(self._gpu_macd[1].cpu()) if hasattr(self, "_gpu_macd") else 0.0
            )
            obv = float(self._gpu_obv.cpu()) if hasattr(self, "_gpu_obv_new") else 0.0
            hawkes = (
                float(self._gpu_hawkes.cpu()) if hasattr(self, "_gpu_hawkes") else 0.0
            )
        else:
            # CPU fallback
            rsi, atr, macd, macd_signal, obv, hawkes = self._compute_cpu_indicators(
                tick
            )
    ...
    async def _compute_returns_gpu(self) -> tuple[float, float, float]:
        ...
        prices = torch.tensor([float(t.mid) for t in self._ticks_buffer], device="cuda")
    ...
    def get_gpu_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "memory_pool_tensors": len(self._memory_pool._in_use)
