...
class EquityCurveGenerator:
    ...
    def calculate_metrics(self) -> Dict:
        ...
        total_return = (
            (equities[-1] - self.initial_capital) / self.initial_capital * 100
        )
    ...
    def plot_plotly(
        self, title: str = "HOPEFX Equity Curve", include_drawdown: bool = True
    ) -> go.Figure:
        """
    ...
    def plot_matplotlib(
        self, title: str = "HOPEFX Equity Curve", figsize: tuple = (12, 8)
    ) -> plt.Figure:
        """
    ...
    def generate_report(
        self, output_dir: Union[str, Path], strategy_name: str = "XAUUSD_ML_Strategy"
    ) -> Dict:
        """
...
<body>
    ...
    def _calculate_monthly_returns(self) -> Dict:
        ...
        return {"years": [str(y) for y in years], "months": months, "values": values}
                ...
                periods.append(
                    {
                        ...
                        "max_drawdown_pct": max(
                            p["drawdown_pct"]
                            for p in self.equity_curve[start_idx : i + 1]
                        ),
                        "duration_days": (
                            point["timestamp"]
                            - self.equity_curve[start_idx]["timestamp"]
                        ).days,
    ...
    def plot_rolling_metrics(self, window: int = 30) -> go.Figure:
        ...
        [p["timestamp"] for p in self.equity_curve[window:]]
