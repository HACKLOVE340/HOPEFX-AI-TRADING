...
class CopulaStressModel:
    ...
    def fit(self, returns: pd.DataFrame) -> None:
        ...
        uniform_data = returns.apply(lambda x: stats.rankdata(x) / (len(x) + 1))
        ...
        logger.info(
            f"Fitted copula with {len(self.copula_params['families'])} pair copulas"
        )
    ...
    def simulate_stress(
        ...
        if conditional_asset and conditional_shock is not None:
            ...
            samples = self._conditional_simulate(
                conditional_asset, conditional_shock, n_sims
            )
        else:
        ...
        returns = pd.DataFrame(samples, columns=self.assets)
    ...
    def _conditional_simulate(
        self, conditional_asset: str, conditional_shock: float, n_sims: int
    ) -> np.ndarray:
...
class StressTester:
    ...
    def _run_scenario(
        ...
        return {
            ...
            "breaches": self._check_limit_breaches(
                total_impact, float(portfolio_value)
            ),
    ...
    def _run_copula_stress(
        ...
        return {
            ...
            "cvar_99": float(
                np.mean(portfolio_pnl[portfolio_pnl <= np.percentile(portfolio_pnl, 1)])
            ),
            "cvar_95": float(
                np.mean(portfolio_pnl[portfolio_pnl <= np.percentile(portfolio_pnl, 5)])
            ),
            "tail_dependence": self._estimate_tail_dependence(simulated_returns),
            "probability_of_ruin": float(
                np.mean(portfolio_pnl < -float(portfolio_value) * 0.5)
            ),
            "expected_shortfall_extreme": float(
                np.mean(
                    portfolio_pnl[portfolio_pnl <= np.percentile(portfolio_pnl, 0.1)]
                )
            ),
        }

    def _run_regime_simulation(
        self, portfolio_value: Decimal, positions: dict[str, Decimal]
    ) -> dict:
        ...
        return {
            ...
            "probability_regime_switch": float(
                1 - self.regime_model.regimes[0].persistence
            ),
        }
    ...
    def _analyze_tail_risk(
        ...
        alpha = 0.05
        weights = np.exp(
            -np.arange(len(sorted_returns)) / (alpha * len(sorted_returns))
        )
        ...
        return {
            ...
            "recovery_50pct_drawdown_days": self._estimate_recovery_time(
    ...
    def _calculate_portfolio_returns(
        self, positions: dict[str, Decimal], returns: pd.DataFrame
    ) -> np.ndarray:
    ...
    def _estimate_recovery_time(
        self, returns: np.ndarray, drawdown_threshold: float
    ) -> int:
    ...
    def _generate_summary(self, results: dict) -> dict:
        ...
        all_scenario_impacts = [
            s["portfolio_impact_pct"] for s in results.get("scenario_analysis", [])
        ]

        return {
            "worst_scenario": min(all_scenario_impacts) if all_scenario_impacts else 0,
            "worst_scenario_name": (
                results["scenario_analysis"][np.argmin(all_scenario_impacts)][
                    "scenario_name"
                ]
                if all_scenario_impacts
                else "N/A"
            ),
            "average_scenario_impact": np.mean(all_scenario_impacts)
            if all_scenario_impacts
            else 0,
            "var_99": results.get("monte_carlo", {}).get("var_99", 0),
            "probability_of_ruin": results.get("monte_carlo", {}).get(
    ...
    def _generate_recommendations(self, results: dict) -> list[str]:
        ...
        if any(
            "correlation_breakdown" in s.get("shock_type", "")
