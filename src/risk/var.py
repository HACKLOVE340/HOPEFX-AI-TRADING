...
class VaRCalculator:
    ...
    def calculate(
        ...
        simulated_returns = self._monte_carlo_sim(returns_history, correlation_matrix)

        # Calculate P&L distribution
        portfolio_values = self._calculate_portfolio_pnl(positions, simulated_returns)
    ...
    def _monte_carlo_sim(
        self, returns: np.ndarray, corr: np.ndarray | None
    ) -> np.ndarray:
        ...
        if corr is not None:
            ...
            simulated = np.random.normal(mean, std, (self.n_sims, len(mean)))

        return simulated

    def _calculate_portfolio_pnl(
        self, positions: list[dict], sim_returns: np.ndarray
    ) -> np.ndarray:
