...
class RiskManager:
    ...
    async def update_price(self, symbol: str, price: Decimal) -> None:
        ...
        if self.account:
            ...
            daily_return = (
                self._daily_pnl / self.account.balance
    ...
    def _calculate_exposure(self) -> dict[str, float]:
        ...
        total_position_value = sum(
            p.quantity * p.entry_price for p in self.account.open_positions.values()
        )

        return {
            "total_position_value": float(total_position_value),
            "exposure_pct": float(total_position_value / self.account.equity)
    ...
    async def _emit_risk_event(self, rule: str, message: str) -> None:
        ...
        await self._event_bus.emit(
            Event.create(
                RiskEvent(
                    level=RiskLevel.HIGH.name, metric=rule, value=0.0, threshold=0.0
                ),
                source="risk_manager",
                priority=2,
            )
