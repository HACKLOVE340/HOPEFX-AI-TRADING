...
class AdaptiveEnsemble:
    ...
    def __init__(self) -> None:
        ...
        self.weights: dict[str, float] = {"xgb": 0.4, "rf": 0.35, "river": 0.25}
        self.performance: dict[str, list[float]] = {"xgb": [], "rf": [], "river": []}
        self.window_size = 100

        # Online learning
        self.river = preprocessing.StandardScaler() | linear_model.LinearRegression(
    ...
    async def predict(
        ...
        ensemble_pred = sum(
            p.prediction * weights.get(p.model, 0) / total_weight for p in predictions
        )

        # Uncertainty propagation
        variances = [p.uncertainty**2 for p in predictions]
        ensemble_var = np.average(
            variances, weights=[weights.get(p.model, 0) for p in predictions]
        )
        ...
        return {
            ...
            "confidence": 1.0
            - min(ensemble_unc / abs(ensemble_pred) if ensemble_pred != 0 else 0, 1.0),
    ...
    async def online_update(
        self, features: dict[str, float], target: float, outcome: float | None = None
    ) -> None:
    ...
    async def _initialize_fresh(self) -> None:
        ...
        self.models["rf"] = RandomForestRegressor(n_estimators=100, max_depth=10)
