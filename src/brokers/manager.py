...
class BrokerManager:
    ...
    async def place_order(
        self, order: Order, broker_name: Optional[str] = None
    ) -> Dict:
        ...
        if broker_name and broker_name in self._brokers:
            ...
            brokers_to_try.extend(
                [n for n in self._brokers.keys() if n not in brokers_to_try]
            )
        ...
        for name in brokers_to_try:
            ...
            try:
                ...
                cb.record_success()
                logger.info(
                    "order_placed", broker=name, symbol=order.symbol, side=order.side
                )
    ...
    async def get_quote(
        self, symbol: str, broker_name: Optional[str] = None
    ) -> TickData:
        ...
        for name in brokers_to_try:
            ...
            try:
                ...
                cb.record_failure()
                logger.warning(
                    "quote_fetch_failed", broker=name, symbol=symbol, error=str(e)
                )
