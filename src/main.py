...
def cli():
    ...
    configure_logging(log_level=settings.log_level.value, json_output=False)
...
def trade(mode: str):
    ...
    async def run():
        ...
        strategy = XAUUSDMLStrategy(
            ...
            parameters={"lookback": 100, "threshold": 0.65, "cooldown": 15},
        )

        # Trading engine
        engine = TradingEngine(broker=broker, data_feed=feed, strategies=[strategy])
