...
class CopyTradingEngine:
    ...
    async def start(self) -> None:
        ...
        await self._event_bus.subscribe("leader_trade", self._on_leader_trade)
    ...
    async def register_copier(self, config: CopyTrader) -> None:
        ...
        logger.info(
            f"Registered copier {config.trader_id} for leader {config.leader_id}"
        )
    ...
    async def _on_leader_trade(self, event: Event) -> None:
        ...
        for copier in self.copiers[leader_id]:
            ...
            # Emit copy order
            await self._event_bus.emit(Event.create(copy_order, source="copy_trading"))
