...
METRICS = {
    ...
    "orders_total": Counter(
        "hopefx_orders_total", "Total orders", ["status", "side", "symbol"]
    ),
    "position_size": Gauge(
        "hopefx_position_size_lots", "Current position size", ["symbol", "direction"]
    ),
    "unrealized_pnl": Gauge("hopefx_unrealized_pnl_usd", "Unrealized P&L", ["symbol"]),
    ...
    "model_drift": Gauge(
        "hopefx_model_drift_score", "Data drift detection score", ["model"]
    ),
    ...
    "redis_ops_total": Counter(
        "hopefx_redis_ops_total", "Redis operations", ["operation", "status"]
    ),
    "db_connections": Gauge("hopefx_db_connections_active", "Active DB connections"),
    "event_queue_depth": Gauge(
        "hopefx_event_queue_depth", "Event bus queue depth", ["priority"]
    ),
}
...
class MetricsExporter:
    ...
    def __init__(self) -> None:
        ...
        # Set static info
        METRICS["info"].info({"version": "6.0.0", "environment": "production"})
    ...
    def record_order(
        ...
        METRICS["orders_total"].labels(
            status=status, side=side.lower(), symbol=symbol
        ).inc()
    ...
    def record_position(
        ...
        METRICS["position_size"].labels(symbol=symbol, direction=direction).set(size)
    ...
    def record_risk(self, var_95: float, drawdown_pct: float, risk_level: int) -> None:
        """Record risk metrics."""
