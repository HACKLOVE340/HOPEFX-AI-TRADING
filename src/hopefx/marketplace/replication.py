...
class CopyTradingEngine:
    ...
    def __init__(
        self, leader_account: Decimal, follower_account: Decimal, oms: OrderManager
    ) -> None:
    ...
    async def on_leader_trade(self, leader_order: OrderEvent) -> None:
        ...
        async with self._lock:
            ...
            # Apply multiplier
            follower_size = (
                Decimal(str(leader_order.quantity))
                * equity_ratio
                * self._ratio.multiplier
            )

            # Apply limits
            max_size = (
                self._follower_equity
                * self._ratio.max_position_pct
                / Decimal(str(leader_order.price or 2000))
            )
            ...
            if estimated_slippage > Decimal(
                str(self._ratio.slippage_tolerance_bps / 10000)
            ):
                logger.warning(
