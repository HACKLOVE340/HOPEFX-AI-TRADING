...
class EventDrivenBacktester:
    ...
    async def _process_signal(self, signal: dict, market: TickData | BarData) -> None:
        ...
        order = await self._oms.submit_order(
            symbol=symbol, side=side, quantity=size, order_type=OrderType.MARKET
        )
    ...
    def _calculate_slippage(self, order: Order, market: TickData | BarData) -> Decimal:
    ...
    async def _close_position(self, symbol: str, market: TickData | BarData) -> None:
    ...
    def _calculate_metrics(self) -> BacktestResult:
        ...
        if len(equity_df) > 1:
            ...
            sortino = (
                np.sqrt(252) * equity_df["returns"].mean() / downside.std()
                if len(downside) > 0
                else 0
            )
        else:
