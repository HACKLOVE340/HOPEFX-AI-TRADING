...
class DeadLetterQueue:
    ...
    def __init__(
        self, max_retries: int = 5, base_delay: int = 60, max_delay: int = 3600
    ) -> None:
        ...
        self._metrics: dict = {"retried": 0, "succeeded": 0, "permanent_failures": 0}
    ...
    async def enqueue(
        ...
        self._failed_events[event_id] = {
            ...
            "handler": handler,
            "attempts": attempt,
            "first_failure": existing["first_failure"]
            if existing
            else datetime.now(timezone.utc),
            "next_retry": next_retry,
            "context": context or {},
            "error_history": existing["error_history"] + [error]
            if existing
            else [error],
        }
    ...
    async def _retry_loop(self) -> None:
        ...
        while True:
            try:
                ...
                ready = [
                    (eid, data)
                    for eid, data in self._failed_events.items()
                ...
                for event_id, data in ready:
                    ...
                    try:
                        ...
                        await self.enqueue(
                            data["event"], str(e), data["handler"], data["context"]
                        )
                        self._metrics["retried"] += 1
    ...
    async def _permanent_failure(
        self, event: Event, error: str, attempts: int, context: Optional[dict]
    ) -> None:
        ...
        logger.error(
            "dlq.permanent_failure", event_id=event.id, attempts=attempts, error=error
        )
    ...
    async def get_metrics(self) -> dict:
        ...
        pending = sum(
            1 for data in self._failed_events.values() if data["next_retry"] > now
        )
        ...
        avg_attempts = (
            sum(d["attempts"] for d in self._failed_events.values())
        ...
        oldest_failure = min(
            (d["first_failure"] for d in self._failed_events.values()), default=None
        )
    ...
    async def archive_permanent_failures(self, storage: Any) -> int:
        ...
        for failure in list(self._permanent_failures):
            ...
            if failure_time < cutoff:
                await storage.store(f"dlq:{failure['event']['id']}", failure)
    ...
    async def _cleanup_permanent_failures(self) -> None:
        ...
        self._permanent_failures = [
            f
            for f in self._permanent_failures
