...
class Wallet(SQLModel, table=True):
    ...
    frozen_balance: Decimal = Field(
        default=Decimal("0"), sa_column=Column(Numeric(19, 8))
    )
...
class Strategy(SQLModel, table=True):
    ...
    performance: Optional["StrategyPerformance"] = Relationship(
...
class StrategyPerformance(SQLModel, table=True):
    ...
    total_return_pct: Decimal = Field(
        default=Decimal("0"), sa_column=Column(Numeric(10, 4))
    )
...
class Trade(SQLModel, table=True):
    ...
    exit_price: Optional[Decimal] = Field(
        default=None, sa_column=Column(Numeric(19, 8))
    )
    ...
    prediction_confidence: Optional[Decimal] = Field(
...
class CopyTrading(SQLModel, table=True):
    ...
    max_drawdown_stop: Decimal = Field(
        default=Decimal("0.10")
    )  # Stop copying at 10% DD
...
class LeaderboardEntry(SQLModel, table=True):
    ...
    total_copied_volume: Decimal = Field(
        default=Decimal("0"), sa_column=Column(Numeric(19, 8))
    )
