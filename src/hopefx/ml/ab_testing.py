...
class ABTestManager:
    ...
    def create_test(self, test_name: str, variants: List[Variant]) -> None:
        """Create new A/B test."""
        ...
        logger.info(
            "ab_test_created", test_name=test_name, variants=[v.name for v in variants]
        )
    ...
    def record_outcome(
        self, test_name: str, variant_name: str, metric_name: str, value: float
    ) -> None:
    ...
    def get_test_results(self, test_name: str) -> Dict:
        ...
        return {
            name: {
                ...
                "metrics": {
                    k: v for k, v in v.metrics.items() if not k.endswith("_count")
                },
            }
