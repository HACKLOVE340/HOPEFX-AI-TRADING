...
class LSTMOnlineModel(BaseModel):
    ...
    async def fit(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        self.model = AttentionLSTM(
            input_size=X.shape[-1], hidden_size=128, num_layers=2
        ).to(self.device)
        ...
        for epoch in range(100):
            ...
            for i in range(0, len(sequences), self.batch_size):
                batch_x = torch.FloatTensor(sequences[i : i + self.batch_size]).to(
                    self.device
                )
                batch_y = torch.LongTensor(targets[i : i + self.batch_size]).to(
                    self.device
                )
    ...
    async def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        with torch.no_grad():
            ...
            x_tensor = (
                torch.FloatTensor(X[-self.sequence_length :])
                .unsqueeze(0)
                .to(self.device)
            )
