...
class EnsembleModel(BaseModel):
    ...
    async def predict(self, features: np.ndarray) -> Prediction:
        ...
        if len(features) >= self.lookback * self.input_size:
            ...
            seq_scaled = self.scaler.transform(
                seq.reshape(-1, self.input_size)
            ).reshape(1, self.lookback, self.input_size)
            seq_tensor = torch.tensor(
                seq_scaled, dtype=torch.float32, device=self.device
            )
    ...
    async def train(self, X: np.ndarray, y: np.ndarray) -> dict[str, float]:
        ...
        example_input = torch.randn(
            1, self.lookback, self.input_size, device=self.device
        )
