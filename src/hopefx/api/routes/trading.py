...
from hopefx.api.dependencies import get_current_user, require_roles, User
...
async def create_order(
    ...
    if not user.has_permission("trading:write"):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Trading permission required"
        )
...
async def list_orders(
    user: User = Depends(get_current_user), oms: OrderManager = Depends()
):
