...
class BoundedEventQueue:
    ...
    async def put(
        self, event: TradingEvent, block: bool = True, timeout: float = 1.0
    ) -> bool:
        ...
        try:
            ...
            if self._dropped % 100 == 1:
                logger.warning(
                    f"Queue {self._name} full, dropped {self._dropped} events"
                )
            return False
...
class EventBus:
    ...
    def subscribe(
        ...
        if type_name not in self._subscribers:
            ...
            self._queues[type_name] = BoundedEventQueue(
                maxsize=queue_size, name=type_name
            )
    ...
    async def _safe_handler(
        self, handler: Callable[[Any], Coroutine[Any, Any, None]], event: TradingEvent
    ) -> None:
        ...
        try:
            ...
            logger.error(
                f"Handler failed for {type(event).__name__}: {e}", exc_info=True
            )
