...
class TradingEngine:
    ...
    async def _subscribe_events(self) -> None:
        ...
        # Market data events
        self._event_bus.subscribe(TickReceived, self._on_tick, priority=10)

        # Signal events
        self._event_bus.subscribe(SignalGenerated, self._on_signal, priority=5)

        # Position events
        self._event_bus.subscribe(PositionOpened, self._on_position_opened, priority=3)
        self._event_bus.subscribe(PositionClosed, self._on_position_closed, priority=3)

        # Risk events
        self._event_bus.subscribe(
            KillSwitchTriggered, self._on_kill_switch_event, priority=1
        )
    ...
    async def _on_position_closed(self, event: Event[PositionClosed]) -> None:
        ...
        logger.info(
            f"Position closed: {event.payload.position_id} P&L: {event.payload.realized_pnl}"
        )
