...
class ByzantineReconciler:
                ...
                diffs.append(
                    {
                        ...
                        "present_in": present,
                        "missing_from": list(
                            set(["oms", "broker", "ledger"]) - set(present)
                        ),
                    }
                )
    ...
    async def _handle_discrepancy(
        self, diffs: list[dict], oms: dict, broker: dict, ledger: dict
    ) -> None:
        ...
        byzantine = [
            d for d in diffs if d["severity"] == ReconciliationSeverity.BYZANTINE
        ]
        critical = [
            d for d in diffs if d["severity"] == ReconciliationSeverity.CRITICAL
        ]
    ...
    async def _emergency_kill(self, reason: str) -> None:
        """Trigger kill switch."""
        await kill_switch.kill(reason=reason, source=KillSource.RECONCILIATION)
