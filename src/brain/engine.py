...
class BrainEngine:
    ...
    async def on_tick(self, event: TickEvent) -> Decision | None:
        ...
        try:
            predictions = await self.ensemble.predict(
                features, regime=regime, regime_confidence=regime_conf
            )
        ...
        stop, take = self._calculate_stops(
            signal, entry, features.atr, predictions["confidence"], regime
        )
    ...
    def _generate_signal(
        self, predictions: dict[str, Any], regime: MarketRegime
    ) -> SignalType:
    ...
    async def _handle_model_failure(self, error: Exception) -> None:
        ...
        if self._consecutive_errors >= self._max_errors:
            await kill_switch.kill(
                f"Model failure cascade: {error}", source=KillSource.DRIFT
            )
