...
class PolygonDataFeed(DataFeed):
    ...
    def __init__(
        self, symbols: list[str], api_key: str | None = None, use_websocket: bool = True
    ):
    ...
    async def _fetch_quote(self, symbol: str) -> TickData | None:
        ...
        async with self._rest_session.get(url) as response:
            ...
            return TickData(
                ...
                mid=(
                    Decimal(str(last.get("bid", 0))) + Decimal(str(last.get("ask", 0)))
                )
                / 2,
                volume=0,
                source="POLYGON_REST",
    ...
    async def get_historical(
        self, symbol: str, start: datetime, end: datetime, timeframe: str = "minute"
    ) -> list[OHLCV]:
