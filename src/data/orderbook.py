...
class PriceLevel:
    ...
    timestamp: float = field(
        default_factory=lambda: asyncio.get_event_loop().time(), compare=False
    )
...
class OrderBook:
    ...
    def update(
        self, side: Side, price: Decimal, volume: Decimal, is_delete: bool = False
    ) -> None:
        """Update price level."""
...
class MultiBookAggregator:
    ...
    def update(
        self, venue: str, symbol: str, side: Side, price: Decimal, volume: Decimal
    ) -> None:
    ...
    def get_consolidated(self, symbol: str) -> dict[str, Any]:
        ...
        # Weighted mid price
        sum(
            self._venue_weights.get(k.split(":")[0], 1.0)
            for k in self.books
            if symbol in k
        )
        ...
        return {
            ...
            "best_global_bid": max(
                (b.best_bid for b in relevant if b.best_bid),
                key=lambda x: x.price,
                default=None,
            ),
            "best_global_ask": min(
                (b.best_ask for b in relevant if b.best_ask),
                key=lambda x: x.price,
                default=None,
            ),
        }
