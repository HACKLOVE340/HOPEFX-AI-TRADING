...
class RedisStreamBus:
    ...
    async def connect(self) -> None:
        ...
        for attempt in range(max_retries):
            try:
                ...
                wait = 2**attempt
                logger.warning(
                    f"Redis connection attempt {attempt + 1} failed, retrying in {wait}s..."
                )
    ...
    async def publish(
        ...
        await self._redis.publish(
            f"pubsub:{topic}",
            json.dumps({"id": message_id, "payload": payload, "priority": priority}),
        )
    ...
    async def _subscribe_stream(
        ...
        for suffix in ["critical", "low"]:
            try:
                await self._redis.xgroup_create(
                    f"{stream_name}:{suffix}", group_name, id="0", mkstream=True
                )
        ...
        task = asyncio.create_task(
            self._consume_stream(
                stream_name, group_name, consumer_name, handler, batch_size, block_ms
            ),
    ...
    async def _consume_stream(
        ...
        streams = [f"{stream_name}:critical", stream_name, f"{stream_name}:low"]
    ...
    async def _subscribe_pubsub(self, topic: str, handler: Callable) -> None:
    ...
    async def _send_to_dlq(
        self, stream: str, msg_id: str, fields: dict, error: str
    ) -> None:
        ...
        await self._redis.xadd("stream:dead_letter_queue", dlq_entry, maxlen=10000)

    async def claim_pending_messages(
        self, topic: str, consumer_group: str, idle_time_ms: int = 60000
    ) -> List[StreamMessage]:
        """
