...
class IntraTradeMonitor:
    ...
    def on_open(self, position: OpenPosition) -> None:
        ...
        logger.debug(
            ...
            position.position_id, position.side, position.symbol,
            position.lots, position.entry_price,
        )
    ...
    def _compute_cvar_pct(self) -> float:
        ...
        total_exposure = sum(
            abs(pos.entry_price * pos.lots * 100.0)
            for pos in self._positions.values()
        )
        if total_exposure <= 0:
            return 0.0
    ...
    def _record_unwind(self, sig: UnwindSignal) -> None:
        ...
        logger.warning(
            ...
            sig.urgency, sig.position_id, sig.reason, sig.mtm_pnl,
        )
