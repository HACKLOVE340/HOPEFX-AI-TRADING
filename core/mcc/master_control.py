...
class MasterControlCore:
    ...
    def __init__(self, config: MCCConfig | None = None):
        ...
        logger.info("HOPEFX MASTER CONTROL CORE v2.0 initialising")
    ...
    def initialize(
        ...
        logger.info("Connecting infrastructure: ConfigManager=%s, Cache=%s, DB=%s",
                     type(config_manager).__name__, type(cache).__name__,
                     "Connected" if db_session else "Not connected")
    ...
    def activate_strategy(self, name: str):
        ...
        if name in self.strategies:
            ...
            logger.info("Strategy activated: %s", name)
    ...
    def deactivate_strategy(self, name: str, reason: str = ""):
        ...
        if name in self.strategies:
            ...
            logger.info("Strategy deactivated: %s%s", name, f" ({reason})" if reason else "")
    ...
    def _on_strategy_signal(self, strategy_name: str, signal: StrategySignal):
        ...
        # Log signal
        logger.info("[%s] Signal: %s (strength: %.2f, conf: %.2f)",
                    strategy_name, signal.action, signal.strength, signal.confidence)
    ...
    def _execute_signal(self, composite: dict):
        """Send to execution"""
        logger.info("Executing: %s (confidence: %.2f)", composite["action"], composite["confidence"])
