...
class ArbitrageExecutor:
    ...
    async def execute(
        ...
        """
        logger.info("Executing: %s -> %s | Profit: %.2f (%.1f bps)",
                    opportunity.buy_exchange, opportunity.sell_exchange,
                    opportunity.net_profit, opportunity.profit_bps)
        ...
        if buy_result and buy_result["filled"] and not sell_result:
            logger.warning("Buy filled, sell failed. Initiating emergency hedge")
        ...
        if sell_result and sell_result["filled"] and not buy_result:
            logger.warning("Sell filled, buy failed. Initiating emergency hedge")
            await self._emergency_hedge(
                sell_ex,
                opportunity.symbol,
                opportunity.size,
        ...
        logger.error("Both legs failed for arbitrage opportunity")
        return False
    ...
    async def _emergency_hedge(self, exchange, symbol, size, side):
        ...
        logger.warning("Emergency %s %s %s on %s", side, size, symbol, exchange.name)
