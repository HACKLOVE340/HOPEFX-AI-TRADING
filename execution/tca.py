...
class MarketImpactModel:
    ...
    def calculate(
        ...
        participation_rate = (
            float(order_size / avg_daily_volume) if avg_daily_volume > 0 else 0
        )
...
class TCAEngine:
    ...
    async def complete_order(self, order_id: str, status: str = "FILLED") -> TCAMetrics:
        ...
        # Calculate benchmark
        await self._get_benchmark_price(
            order["symbol"],
            order["benchmark"],
            order["arrival_time"],
            last_fill.timestamp,
    ...
    def get_stats(self, n: int = 100) -> dict[str, Any]:
        ...
        return {
            ...
            "alpha_positive": len([m for m in recent if m.alpha_extraction_bps > 0])
            / len(recent),
        }
