...
def main() -> None:
    ...
    schema = _minimal_schema() if args.no_import else get_openapi_schema()
    ...
    endpoint_count = sum(
        len([m for m in methods if isinstance(methods[m], dict)]) for methods in schema.get("paths", {}).values()
    )
