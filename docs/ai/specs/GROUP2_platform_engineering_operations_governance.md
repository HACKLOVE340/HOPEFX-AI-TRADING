...
| 2 | Rule 1 injection evidence across existing gates | 0, 19, 20 | **Critical** | **PARTIAL — Phases R2–R6.** Ledger built and ratcheting: 23 gates, **17 proven, 6 unproven**. Injecting found further dead controls — gate M passed with no dataset, the secret scanner skipped credentials containing `xxx` or `none`, and gate E's dead-file detector had never actually detected a dead file in any guarded package |
