...
> **For agentic workers:** steps use checkbox (`- [x]`) syntax. Each task ends
...
- [x] Test: a broker that raises leaves `_hedge_active` False and appends no position.
- [x] Test: a second call after a failure actually retries (the guard must not latch).
- [x] Test: with no broker, no `HedgePosition` is recorded.
- [x] Test: a successful placement still sets the flag, the position and the gauge.
- [x] Test: the endpoint does not return `status: "ok"` for a hedge that failed.
- [x] Implement: `activate_hedge_mode() -> bool`; state mutated only after a
...
- [x] Test: each of the five verified false positives scores 0 / `normal`.
- [x] Test: the true positives still match ("a coup in", "nuclear war", …),
      including multi-word terms and terms adjacent to punctuation.
- [x] Test: matching stays case-insensitive.
- [x] Implement: compile each term to `\b<escaped>\b` once at load; keep the
...
- [x] Test: a missing feature reaches the model at the training mean (z≈0), not
      at `-mean/std`.
- [x] Test: a present feature is scaled exactly as before (no regression).
- [x] Test: `scaler.transform` raising no longer passes the **unscaled** frame
      to the learners.
- [x] Implement: mark which columns were absent, scale, then set those columns
...
- [x] Test: one year of hourly bars at +100% reports ≈100%, not 2.93%.
- [x] Test: Calmar equals the corrected annual return over max drawdown.
- [x] Test: a daily-bar config (`bars_per_day=1`) is unchanged.
- [x] Implement: divide the bar count by `self.config.bars_per_day`.
- [x] Commit.
...
- [x] Test: against a hand-computed downside deviation on a fixed series.
- [x] Test: a series with no losing period does not divide by zero.
- [x] Test: both engines return the same Sortino for the same returns.
- [x] Implement: `sqrt(mean(minimum(r - target, 0) ** 2))` in both, target 0.
- [x] Commit.
...
- [x] Test: on a rising series the EMA is nearer the newest close than the
      oldest (this is what fails today).
- [x] Test: against a hand-computed EMA on a fixed series.
- [x] Implement: drop `reversed()`; seed from the first bar of the window.
- [x] Commit.
...
All six fixed, each with a red-green test cycle. Three findings surfaced while
doing it — F252, F253, F254 — recorded in `CODE_READING_FINDINGS.md`.

Two tasks turned out to be larger than filed:

* **Task 1 (F81)** grew to cover `deactivate_hedge_mode`, which had the mirror
  defect and a worse consequence (F252), and to correct seven existing tests
  that asserted the defect as the requirement or relied on it.
* **Task 2 (F80)** needed two mechanisms, not one: word boundaries fix four of
  the five verified false positives, and the remaining two are genuine
  whole-word matches whose *sense* is wrong (F253).

One assertion of mine was wrong where the code was right (F254) and is corrected
in the test.
