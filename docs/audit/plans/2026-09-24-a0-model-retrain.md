...
- Run no training command with the main checkout as its CWD. Both retrain scripts default to `ml/saved_models/`. Since 2026-09-24, `retrain_horizon5.py` accepts `--output-dir` (or `RETRAIN_OUTPUT_DIR`) and `train_advanced.py` accepts `--model-dir`, so pass one of these for every rehearsal. `ML_MODEL_DIR` is still deliberately ignored.
...
### Dry-run defects — fixed 2026-09-24 (owner-approved)

All three are test-first; each new test was run against the pre-fix files and failed.

1. **Rollback pointer.** `rollback()` now repoints `current.pkl` from `file`, the one field `register()` writes (no second field was introduced). Relative `file` values, as in the shipped registry, resolve against the repository root from any working directory. A missing artifact refuses **before** the manifest changes, and a pointer that does not resolve to the restored artifact raises. The link is now written relative (`-> advanced_oos.pkl`, like the committed one): the old absolute link would have broken on clone and failed `verify_model` in CI after Task 8's commit. Tests: `tests/unit/test_registry_rollback_restores_pointer.py`.
2. **One state vocabulary: `"active"`.** `ml.model_registry.STATE_ACTIVE` is what `promote`/`rollback` write and what `ml/verify_model.py` requires, and it is what the shipped `registry.json` already holds. `verify_model` is unchanged and still refuses anything else. Readers that act on the serving model accept legacy `"production"` via `is_active_state()`: `promote` (retire previous), `ml/sharpe_circuit_breaker.py::_retire_model` (it never retired the shipped `"active"` model), and `api/superadmin/ml_ai.py` (it listed the serving model as "staged"). `register()` also gained `sharpe=`, because `verify_model`'s Sharpe floor reads `entry["sharpe"]` and no registered entry could carry it.
3. **Retrain data and output.** `retrain_horizon5.py` takes `--cached-csv` (default `ml.cached_series.DEFAULT_FILES["XAUUSD"][0]` = `data/XAUUSD_40Y.csv`) and always passes `--use-cached --cached-csv … --no-download --model-dir …` to `train_advanced.py`. An explicit `--cached-csv` is now the only file tried, and `--no-download` turns the yfinance/`GC=F` fallback into a `FileNotFoundError`. `--output-dir` / `RETRAIN_OUTPUT_DIR` (default `ml/saved_models`) redirects every write. `ML_MODEL_DIR` is deliberately not read (HARDENING_BACKLOG). A redirected run archives against its own `registry.json`, never the shipped one. A `--years` window before `CLEAN_SINCE` logs a warning. It is not refused, because the window is the owner's call.
   - **Proved by execution:** `retrain_horizon5.py --smoke --output-dir <scratch>` ran with yfinance import-blocked, loaded 377 bars from `XAUUSD_40Y.csv`, and wrote only to the scratch directory. The worktree `ml/saved_models/*` sha256 was unchanged.
   - **New finding, not fixed:** `--smoke` makes `train_advanced` run with `--oos-years 0`, so it writes **no** `advanced_oos.pkl`. The smoke "CI GATE" in `retrain.yml` has only ever passed because the committed `ml/saved_models/advanced_oos.pkl` was already there. Redirected, the gate fails with "missing artifact advanced_oos.pkl".
- **Task 5 re-drill (on a copy of the shipped registry):** `promote(drill)` → `current.pkl -> drill_candidate.pkl`, `xgb_horizon5_v3` retired. `rollback("xgb_horizon5_v3")` → `current.pkl -> advanced_oos.pkl`, state `active`. **`verify_model` then passes.**
