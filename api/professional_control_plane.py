...
_config: dict[str, Any] = {
    ...
    "agents": [], "teams": [], "tools": [], "plugins": [], "connectors": [],
...
_readiness: dict[str, Any] = {
    "status": "starting", "started_at": datetime.now(UTC).isoformat(),
    "active_model": _config["models"]["primary"], "components": {}, "degraded_reasons": [],
}
...
def _record(user: TokenPayload, action: str, reason: str, result: str, diff: dict[str, Any] | None = None) -> None:
    _audit.append({"id": f"audit-{len(_audit) + 1}", "actor": user.sub, "action": action,
                   "reason": reason, "result": result, "revision": _config["revision"],
                   "environment": _config["environment"], "diff": diff or {},
...
async def get_readiness(_: TokenPayload = Depends(_admin)) -> dict[str, Any]:
    return {**copy.deepcopy(_readiness), "configuration_revision": _config["revision"],
...
async def apply_configuration(update: ConfigUpdate, user: TokenPayload = Depends(_admin)) -> dict[str, Any]:
    ...
    for key, value in changes.items():
        if isinstance(value, dict) and isinstance(_config.get(key), dict):
            ...
            _config[key] = value
    _config["revision"] += 1
    entry = {"revision": _config["revision"], "actor": user.sub, "reason": update.reason,
             "created_at": datetime.now(UTC).isoformat(), "configuration": _snapshot()}
...
async def reprobe(user: TokenPayload = Depends(_admin)) -> dict[str, Any]:
    ...
    components = {"configuration": "ready", "core_brain": "ready" if _config["core_brain"]["enabled"] else "stopped",
                  "model_registry": "ready", "agents": "ready", "connectors": "ready",
                  "sandbox": "ready" if _config["sandbox"]["enabled"] else "stopped", "trading": "paper"}
    await asyncio.sleep(0)
    _readiness.update({"status": "ready", "components": components, "degraded_reasons": [],
...
async def sandbox_test(test: SandboxTest, user: TokenPayload = Depends(_admin)) -> dict[str, Any]:
    ...
    if _config["sandbox"]["network"] or any(token in test.command.lower() for token in ("curl", "wget", "ssh", "rm -rf")):
    ...
    return {"status": "accepted", "command": test.command, "timeout_seconds": test.timeout_seconds,
            "network": False, "filesystem": _config["sandbox"]["filesystem"]}
