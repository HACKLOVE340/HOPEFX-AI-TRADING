...
async def broker_status():
    ...
    try:
        ...
        logger.exception("broker_status: unexpected error")
        return {
...
async def paper_clock_status():
    ...
    try:
        ...
        _raw: dict = get_clock().status()
        # Mask the account_id — expose only the last 4 chars.
        _raw_id: str = str(_raw.get("account_id") or "")
        _masked_id: str | None = ("..." + _raw_id[-4:]) if len(_raw_id) > 4 else ("****" if _raw_id else None)
        # Return an explicit allowlist of fields so no internal state leaks.
        return {
            "started": bool(_raw.get("started")),
            "elapsed_days": float(_raw.get("elapsed_days") or 0.0),
            "remaining_days": float(_raw.get("remaining_days") or 0.0),
            "complete": bool(_raw.get("complete")),
            "account_id": _masked_id,
            "pending_real_account": bool(_raw.get("pending_real_account")),
            "environment": str(_raw.get("environment") or ""),
        }
    except Exception as exc:
...
async def stamp_oanda_clock(
    ...
    try:
        ...
        logger.exception("stamp_oanda_clock failed")
        from fastapi import HTTPException
